...
A unique ID string which can be used to ensure that the CSS class names dynamically generated by Material UI are unique for a given `<Vitessce/>` component if there are multiple rendered on the HTML page.
