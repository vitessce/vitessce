...
npm run dev
```
Note: react@18.0.0 is compatible with Vitessce
...
```
## Troubleshooting
 if you get error in pushing to main, try the following and commit the changes in the editor:
```
...
For more information about the React component, see the [documentation page](https://vitessce.io/docs/js-react-vitessce/).
...
Edit the `vite.config.js` file to allow Vite to serve correctly from a subpath like `https://yourname.github.io/your-repo-name/`
```js
