...
### `PluginJointFileType(fileType, expansionFunction, optionsSchema)`
...
In previous versions of Vitessce, we provided a global plugin registration API (`registerPluginViewType`, `registerPluginFileType`, etc).
