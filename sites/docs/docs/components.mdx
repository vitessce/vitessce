...
This view wraps [Neuroglancer](https://github.com/google/neuroglancer), a high-performance WebGL2-based viewer developed by Google for huge multi-resolution volumes and segmentations.

<ComponentImage filename="neuroglancer.png" alt="Screenshot of Neuroglancer view" />

In order to use this view, a minimal `viewerState` configuration must be passed as a prop to the view
(e.g., using the `view.setProps({ viewerState: VIEWER_STATE })` [function](/docs/view-config-js/#setpropsprops)).
The complete `viewerState` schema can be found in the [neuroglancer documentation](https://neuroglancer-docs.web.app/json/api/index.html).

```js
const VIEWER_STATE = {
  // Global coordinate system. Values are [scale, unit].
  ...
  // 3D layout.
  layout: "3d",
  // Layers — include at least one that actually draws something.
  layers: [
    {
      name: "image_name",
      type: "image",
...
The requirement to pass the Neuroglancer `viewerState` as a prop is subject to change.
See [#2136](https://github.com/vitessce/vitessce/issues/2136) for more details.
