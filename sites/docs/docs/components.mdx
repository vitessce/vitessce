...
The Neuroglancer view embeds [Neuroglancer](https://github.com/google/neuroglancer) tool developed by Google to display 3D meshes as segments. It's a high-performance, WebGL2-based viewer for huge multi-resolution volumes and segmentations.

<ComponentImage filename="neuroglancer.png" alt="Screenshot of Neuroglancer view" />

The view requires a minimal viewerState configuration with the following properties that can be added to the the Vitessce config using the `setProps({ viewerState: VIEWER_STATE})`.
Here is a a complete list of attributes in the [viewerState](https://neuroglancer-docs.web.app/json/api/index.html).
