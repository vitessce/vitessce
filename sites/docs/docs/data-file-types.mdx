...
See our documentation on how to store point data to enable efficient [tiled loading](/docs/data-troubleshooting/#points) when using SpatialData.
...
The URL should point to "precomputed" **multi-resolution chunked meshes**.
...
This fileType is intended to be used with the `neuroglancer` [view type](../components/#neuroglancer).

The fileType `options` value is used to construct the internal `viewerState` for the Neuroglancer view.
...
Note: Neuroglancer expects coordinates in the precomputed mesh format to be in **nanometers** (XYZ).
See [discussion on GitHub](https://github.com/google/neuroglancer/issues/442#issuecomment-1440550158) for more details.
...
<FileDefTabs
  ...
  options={`{
    ...
    // Precomputed format expects nanometer (nm) units.
