...
<FileDefTabs
fileName="my_sample_adata.zarr"
fileType="sampleSets.anndata.zarr"
fileConst="SAMPLE_SETS_ANNDATA_ZARR"
options={`{
    "sampleSets": [
      {
        "name": "Disease State",
...
<FileDefTabs
fileName="my_comparative_adata.zarr"
fileType="comparisonMetadata.anndata.zarr"
fileConst="COMPARISON_METADATA_ANNDATA_ZARR"
options={`{
...
Per-feature statistics that have been pre-computed and stored in the AnnData object.
Rather than pointing to the location of the results directly,
with this file type, we point to the comparison metadata location
(which should internally contain references to multiple featureStats dataframes,
for example, the results of multiple differential expression tests).

<FileDefTabs
fileName="my_comparative_adata.zarr"
fileType="comparativeFeatureStats.anndata.zarr"
fileConst="COMPARATIVE_FEATURE_STATS_ANNDATA_ZARR"
options={`{
...
Per-feature-set statistics that have been pre-computed and stored in the AnnData object.
Rather than pointing to the location of the results directly,
with this file type, we point to the comparison metadata location
(which should internally contain references to multiple featureSetStats dataframes,
for example, the results of multiple gene set enrichment tests).

<FileDefTabs
fileName="my_comparative_adata.zarr"
fileType="comparativeFeatureSetStats.anndata.zarr"
fileConst="COMPARATIVE_FEATURE_SET_STATS_ANNDATA_ZARR"
options={`{
    // Path to the comparison metadata.
    "metadataPath": "uns/comparison_metadata",
...
Per-observation-set statistics that have been pre-computed and stored in the AnnData object.
Rather than pointing to the location of the results directly,
with this file type, we point to the comparison metadata location
(which should internally contain references to multiple obsSetStats dataframes,
for example, the results of multiple cell type composition analyses).

<FileDefTabs
fileName="my_comparative_adata.zarr"
fileType="comparativeObsSetStats.anndata.zarr"
fileConst="COMPARATIVE_OBS_SET_STATS_ANNDATA_ZARR"
options={`{
    // Path to the comparison metadata.
    "metadataPath": "uns/comparison_metadata",
...
fileConst="OBS_POINTS_SPATIALDATA_ZARR"
options={`{
...
fileConst="SHAPES_SPATIALDATA_ZARR"
options={`{
...
### `obsEmbedding.spatialdata.zarr`

<FileDefTabs
fileName="my_sdata.zarr"
fileType="obsEmbedding.spatialdata.zarr"
fileConst="OBS_EMBEDDING_SPATIALDATA_ZARR"
options={`{
    // Should point to an array of (d1, d2) coordinate pairs, one coordinate pair per obs/cell.
    "path": "obsm/X_umap",
    // Dimension indices are optional. By default, [0, 1].
...
options={`{
    ...
    "obsSets": {
      ...
      // Similar to obsSets.anndata.zarr
      "obsSets": [
        {
          "name": "Cell Type Annotations",
          "path": ["tables/table/obs/cell_type_coarse", "tables/table/obs/cell_type_fine"]
    ...
    "obsEmbedding": [
      {
        // Should point to an array of dimensionality reduction coordinates;
        // Similar to obsEmbedding.anndata.zarr
...
See the [OME-TIFF Troubleshooting](../data-troubleshooting/#ome-tiff) documentation for more information.
...
Optionally, `options.coordinateTransformations` can be defined according to the OME-NGFF v0.4 [coordinateTransformations](https://ngff.openmicroscopy.org/0.4/#trafo-md) spec ([Zod schema](https://github.com/vitessce/vitessce/blob/25f61fa52ab5b10daf84afe9b8e4a8514189f404/packages/schemas/src/file-def-options.ts#L153)).
...
## `obsSegmentations.ng-precomputed`

Points to a URL containing precomputed **multi-resolution chunked meshes** for 3D data rendered in the Neuroglancer view. See the Neuroglancer docs on meshes: [Multi-resolution mesh format](https://github.com/google/neuroglancer/blob/master/src/datasource/precomputed/meshes.md#multi-resolution-mesh-format).

The provided options are used to construct a minimal ViewerState for the Neuroglancer view. The complete `viewerState` schema is available in the [neuroglancer documentation](https://neuroglancer-docs.web.app/json/api/index.html).

**Units note:** Precomputed expects voxel resolutions in **nanometers** (XYZ). See this discussion:  
[Resolution is specified in nanometers per pixel](https://github.com/google/neuroglancer/issues/442#issuecomment-1440550158).

<FileDefTabs
  fileName="my_meshes"
  fileType="obsSegmentations.ng-precomputed"
  fileConst="OBS_SEGMENTATIONS_NG_PRECOMPUTED"
  options={`{
    // dimensions map voxel steps to physical units along each axis.
    // Precomputed expects nanometers (nm) for resolution.
    // If your data are in meters, convert (e.g., 1e-9 m = 1 nm) before setting these.
    dimensionX: 1,      // 1 nm
    dimensionY: 1,      // 1 nm
    dimensionZ: 1,      // 1 nm
    dimensionUnit: 'nm',

    // Optional camera settings; defaults are used if omitted.
    ...
    // Currently only '3d' is supported; defaults to '3d' if omitted.
    layout: '3d'
  }`}
/>
