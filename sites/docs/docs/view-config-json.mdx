...
| `1.0.18` | Update `spatialdata.zarr` config options schema to support an `obsSegmentations` property (replaces usage of `shapes` and `labels`) for consistency with other options schemas. |
| `1.0.19` | Replace `spatial` and `layerController` implementations with `spatialBeta` and `layerControllerBeta`. |
| `1.0.20` | Rename `spatialBeta` and `layerControllerBeta` to `spatial` and `layerController`. |
