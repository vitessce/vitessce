...
We realise that having to construct a whole view config adds a lot of unnecessary friction, especially in the cases when one just wants to view a quick visualization of an image or AnnData object. This is why for some file formats, Vitessce can generate a valid view config, using URLs to your datasets (see [Data Preparation](/docs/#data-preparation)). For more information, see the [default view config](/docs/default-config-json/) page.
