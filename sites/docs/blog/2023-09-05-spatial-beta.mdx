...
Meanwhile, the opacity and visibility properties remain primitive numeric and boolean values, respectively, which are agnostic to the existence of the image layer.
...
![Filled and stroked ("boundaries") cell segmentations](/img/blog/segmentation_boundaries.png)
...
- [Bitmask-based obsSegmentations for functional tissue units](#todo)
- [Bitmask-based obsSegmentations for cells and nuclei](../../#?dataset=codex-2023)

## Usage from Python

We have implemented the object-oriented API for the multi-level coordination model in both JavaScript and Python.
The equivalent of the JS linkViewsByObject function is the Python link_views_by_dict function.
...
This would be a non-breaking change with automatic upgrade functions to internally convert previously-configured visualizations (in the same way we have implemented upgrade functions for all prior introductions of new config schema versions).
