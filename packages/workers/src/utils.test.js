...
describe('heatmap tiling utils', () => {
  it('creates cell x gene tiles (transpose = false)', () => {
    ...
    expect(Array.from(tile00)).toEqual([0, 255, 255, 0, 255, 0, 0, 255, 255]);
    // Tile (0, 1)
    ...
    expect(Array.from(tile01)).toEqual([0, 0, 0, 0, 0, 0, 0, 0, 0]);
    // Tile (1, 0)
    ...
    expect(Array.from(tile10)).toEqual([0, 0, 0, 0, 255, 0, 0, 255, 0]);
  });
  ...
  it('creates gene x cell tile (transpose = true)', () => {
    ...
    expect(Array.from(tile00)).toEqual([255, 0, 255, 255, 255, 255, 0, 0, 0]);
    ...
    expect(Array.from(tile01)).toEqual([0, 0, 0, 255, 255, 0, 0, 0, 0]);
    ...
    expect(Array.from(tile10)).toEqual([255, 255, 255, 0, 0, 0, 0, 0, 0]);
  });
  ...
  it('creates cell x gene tiles (transpose = false) with re-ordered cells', () => {
    ...
    expect(Array.from(tile00)).toEqual([0, 255, 0, 0, 255, 255, 0, 255, 0]);
    // Tile (0, 1)
    const tile01 = getCellByGeneTile(arr, {
      tileSize: 3,
      numCells,
    ...
    expect(Array.from(tile10)).toEqual([0, 0, 0, 0, 0, 0, 0, 255, 255]);
  });
});
