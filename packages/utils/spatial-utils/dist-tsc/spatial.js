...
export const canLoadResolution = (loader, resolution) => {
    const { totalBytes, height, width, depthDownsampled, } = getStatsForResolution(loader, resolution);
    ...
    if (coordinateTransformations && axes) {
        ...
        coordinateTransformations.forEach((transform) => {
            if (transform.type === 'translation') {
                ...
                const xyzTranslation = xyzIndices.map(axisIndex => (axisIndex >= 0
            ...
            if (transform.type === 'scale') {
                ...
                const xyzScale = xyzIndices.map(axisIndex => (axisIndex >= 0
