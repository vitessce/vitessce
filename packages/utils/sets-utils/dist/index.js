...
pointInPolygon$1.exports = function pointInPolygon(point2, vs, start, end) {
...
function nodeTransform$1(node, predicate, transform, transformedPaths, currPath = null) {
  let newPath;
  if (!currPath) {
    newPath = [node.name];
  } else {
  ...
  if ("children" in node) {
    return {
      ...node,
      children: node.children.map((child) => nodeTransform$1(child, predicate, transform, transformedPaths, newPath.concat([child.name])))
    };
  }
  return node;
}
...
function getNextNumberedNodeName(nodes, prefix, suffix) {
  let i = 1;
  if (nodes) {
...
function setObsSelection(cellSelection, additionalCellSets, cellSetColor, setCellSetSelection, setAdditionalCellSets, setCellSetColor, setCellColorEncoding, prefix = "Selection ", suffix = "") {
  const CELL_SELECTIONS_LEVEL_ZERO_NAME = "My Selections";
  const selectionsLevelZeroNode = additionalCellSets == null ? void 0 : additionalCellSets.tree.find(
  ...
  const nextName = getNextNumberedNodeName(selectionsLevelZeroNode == null ? void 0 : selectionsLevelZeroNode.children, prefix, suffix);
  let colorIndex = 0;
...
function getObsInfoFromDataWithinRange(range2, data) {
  const [lowerBound, upperBound] = range2;
  const cellIds = data.filter((item) => item.value >= lowerBound && item.value <= upperBound).map((item) => item.cellId);
  return cellIds;
}
...
function treeToCellSetColorIndicesBySetNames(currTree, selectedNamePaths, cellSetColor) {
  let cellColorsArray = [];
  selectedNamePaths == null ? void 0 : selectedNamePaths.forEach((setNamePath) => {
    const node = treeFindNodeByNamePath(currTree, setNamePath);
    if (node) {
      const nodeSet = nodeToSet(node);
      const nodeColorIndex = cellSetColor == null ? void 0 : cellSetColor.findIndex((d) => isEqual(d.path, setNamePath));
      cellColorsArray = [
        ...cellColorsArray,
        ...nodeSet.map(([cellId]) => [
          cellId,
...
function treeToObjectsBySetNames(currTree, selectedNamePaths, setColor, theme) {
  ...
  for (let i = 0; i < selectedNamePaths.length; i += 1) {
    ...
    if (node) {
      const nodeSet = nodeToSet(node);
      const nodeColor = ((_a = setColor == null ? void 0 : setColor.find((d) => isEqual(d.path, setNamePath))) == null ? void 0 : _a.color) || getDefaultColor(theme);
...
function treeToCellPolygonsBySetNames(currTree, obsIndex, obsEmbedding, selectedNamePaths, cellSetColor, theme) {
    ...
    if (node) {
      ...
      if (cellPositions.length > 2) {
        const points = featureCollection(
          cellPositions.map(point)
        );
        const concavity = Infinity;
        const hullCoords = concaveman$1(cellPositions, concavity);
        if (hullCoords) {
...
function treeToSetSizesBySetNames(currTree, allNamePaths, selectedNamePaths, setColor, theme) {
  const sizes = [];
  ...
  allNamePaths.forEach((clusterPath) => {
    ...
    if (node) {
      const nodeSet = nodeToSet(node);
      const nodeColor = ((_a = setColor == null ? void 0 : setColor.find((d) => isEqual(d.path, clusterPath))) == null ? void 0 : _a.color) || getDefaultColor(theme);
      const nodeProps = {
        key: generateKey(),
        name: node.name,
        size: nodeSet.length,
...
function treesConflict(cellSets, testCellSets) {
  ...
  function getPaths2(node, prevPath) {
    paths.push([...prevPath, node.name]);
    if (node.children) {
      node.children.forEach((c) => getPaths2(c, [...prevPath, node.name]));
    }
  }
  cellSets.tree.forEach((lzn) => getPaths2(lzn, []));
...
function findLongestPath(arrOfPaths, path, isSubset) {
  if (Array.isArray(arrOfPaths)) {
    ...
    arrOfPaths.forEach((subArray) => {
      ...
      if (matchCount === (isSubset ? subArray.length : path.length) && subArray.length > longestLength) {
        longest = subArray;
...
function findLongestCommonPath(arrOfPaths, cellSetSelection) {
  let maxCount = 0;
  let commonPath = [];
...
function filterPathsByExpansionAndSelection(mergedCellSets, hierarchy, cellSetExpansion, cellSetSelection) {
  const paths = getPaths({ children: mergedCellSets.tree });
  const contains = (allPaths, path) => allPaths == null ? void 0 : allPaths.some((p) => isEqual(p, path));
...
function findChangedHierarchy(prevSelectedPaths, currSelectedPaths) {
  ...
  const changedPath = newlySelectedPaths.length > 0 ? newlySelectedPaths[0] : unselectedPaths[0];
        ...
        if (isLeaf) {
          ...
          if (shouldBeLeaf) {
            ...
            resultNode.children = nextLevelNames.map((nextLevelName) => getNode(
              ...
              nextLevelName,
              childLevelSuffixes.filter((l) => l[0] === nextLevelName).map((l) => l.slice(1)).filter((v) => v.length > 0)
            ));
          }
        }
        return resultNode;
