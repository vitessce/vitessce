...
import { nodeToRenderProps, nodeToSet, nodeToHeight, treeFindNodeByNamePath, nodeTransform, treeToUnion, treeToIntersection, treeToComplement, nodeToLevelDescendantNamePaths, treeExport, filterNode, treeToSetSizesBySetNames, } from './cell-set-utils.js';
import { levelTwoNodeLeaf, levelZeroNode, tree, } from './cell-set-utils.test.fixtures.js';
describe('Hierarchical sets cell-set-utils', () => {
    ...
    describe('Alter Node properties', () => {
        it('Node Transform', () => {
            const nodeTransformedWithPredicate = nodeTransform(cloneDeep(levelZeroNode), node => node.name === 'Pericytes', 
            // eslint-disable-next-line no-param-reassign
            (node) => { node.name = 'New name'; return node; }, [], ['Cell Type Annotations']);
            ...
            expect(nodeTransformedWithPredicate.children[0].children[0].name).toEqual('New name');
            // eslint-disable-next-line no-param-reassign
            const nodeTransformedWithoutPredicate = nodeTransform(cloneDeep(levelZeroNode), () => false, 
            // eslint-disable-next-line no-param-reassign
            (node) => { node.name = 'New name'; return node; }, [], ['Cell Type Annotations']);
            // No nodes transformed for fals-y predicate.
            expect(nodeTransformedWithoutPredicate.name).toEqual('Cell Type Annotations');
            expect(nodeTransformedWithoutPredicate.children[0].children[0].name).toEqual('Pericytes');
        });
    });
    describe('Generate properties from paths', () => {
        it('treeToSetSizesBySetNames where one bar is selected', () => {
            ...
            const setSizes = treeToSetSizesBySetNames(tree, [['Cell Type Annotations', 'Vasculature', 'Endothelial'], ['Cell Type Annotations', 'Vasculature']], [['Cell Type Annotations', 'Vasculature', 'Endothelial']], setColor, 'dark');
        ...
        it('treeToSetSizesBySetNames where nothing is selected', () => {
            ...
            const setSizes = treeToSetSizesBySetNames(tree, [['Cell Type Annotations', 'Vasculature', 'Endothelial'], ['Cell Type Annotations', 'Vasculature']], [], setColor, 'dark');
            expect(setSizes.length).toEqual(2);
            expect(setSizes[0].name).toEqual('Endothelial');
            expect(setSizes[0].size).toEqual(3);
            expect(setSizes[0].color).toEqual([255, 0, 0]);
        ...
        it('treeToSetSizesBySetNames where selected paths are not part of all paths', () => {
            ...
            const setSizes = treeToSetSizesBySetNames(tree, [['Cell Type Annotations', 'Vasculature', 'Endothelial'], ['Cell Type Annotations', 'Vasculature']], [['Louvain Clusters', '0'], ['Louvain Clusters', '1']], setColor, 'dark');
            expect(setSizes.length).toEqual(2);
            expect(setSizes[0].name).toEqual('Endothelial');
            expect(setSizes[0].size).toEqual(3);
            expect(setSizes[0].color).toEqual([255, 0, 0]);
    ...
    describe('Get from tree', () => {
        ...
        it('Tree Union Complement', () => {
            const complement = treeToComplement(tree, [
                ['Cell Type Annotations', 'Vasculature', 'Endothelial'],
                ['Cell Type Annotations', 'Vasculature', 'Epithelial', 'Squamous'],
        ...
        it('Node Descendants by Level', () => {
            ...
            expect(descendantsStopEarly).toEqual([['Cell Type Annotations', 'Vasculature', 'Pericytes'],
                ['Cell Type Annotations', 'Vasculature', 'Endothelial'],
