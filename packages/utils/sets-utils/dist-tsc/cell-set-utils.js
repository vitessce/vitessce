...
import { HIERARCHICAL_SCHEMAS, } from './constants.js';
...
export function treeToCellPolygonsBySetNames(currTree, obsIndex, obsEmbedding, selectedNamePaths, cellSetColor, theme) {
 ...
 * and `color`.
 */
export function treeToSetSizesBySetNames(currTree, allNamePaths, selectedNamePaths, setColor, theme) {
    const sizes = [];
    /**
...
export function getCellSetPolygons(params) {
    const { obsIndex, obsEmbedding, cellSets, cellSetSelection, cellSetColor, theme, } = params;
    if (cellSetSelection && cellSetSelection.length > 0 && cellSets && obsIndex && obsEmbedding) {
        return treeToCellPolygonsBySetNames(cellSets, obsIndex, obsEmbedding, cellSetSelection, cellSetColor, theme);
    }
    return [];
}
