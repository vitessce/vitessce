...
import { arrayToString, getViewTypesMap, getAltText } from './generate-alt-text.js';
...
describe('Tests for generating an alt text from the vitessce config', () => {
  ...
  describe('Tests for getViewTypesMap', () => {
    it('should have a natural language mapping for every defined viewtype', () => {
      const mapping = getViewTypesMap();
      const mappingLength = mapping.keys().length;
      expect(mappingLength).toEqual(ViewType.length);
    });

    it('should not have undefined in the mapping', () => {
      const mapping = getViewTypesMap();
      expect(undefined in mapping.keys() || undefined in mapping.values()).toEqual(false);
    });
  });
