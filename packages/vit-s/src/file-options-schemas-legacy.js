...
export const anndataCellsZarrSchema = {
  ...
  properties: {
    xy: {
      type: 'string',
      description:
        "The location in the AnnData store of cell centroids, like 'obsm/X_centroids.'",
    },
    poly: {
      type: 'string',
      description:
        "The location in the AnnData store of cell polygon outlines, like 'obsm/X_polygons.'",
    },
    factors: {
      type: 'array',
      description:
        "List of locations in the AnnData store of cell sets, like 'obs/louvain'",
      items: {
        type: 'string',
      },
    },
    mappings: {
      patternProperties: {
        '.': {
          type: 'object',
          description:
            "An object containing key-values for mappings like { UMAP: { key: 'obsm/X_umap', dims: [0, 1] } }.",
          additionalProperties: false,
          required: ['key'],
          properties: {
            key: {
              type: 'string',
              description:
                "Where to look in the AnnData store for this mapping, like 'obsm/X_umap.'",
            },
            dims: {
              type: 'array',
              description:
...
export const anndataExpressionMatrixZarrSchema = {
  ...
  properties: {
    matrix: {
      type: 'string',
      description:
        "The location in the AnnData store of the cell x gene matrix, like 'obsm/hvg_subset.' or 'X'",
    },
    geneFilter: {
      type: 'string',
      description:
        "The location in the AnnData store of a filter for the genes if using a subset of the data, like 'var.highly_variable.' if the matrix comes from 'obsm/hvg_subset.'",
    },
    matrixGeneFilter: {
      type: 'string',
      description:
        "The location in the AnnData store of a filter for the matrix data (used in heatmap and histogram), like 'var.highly_variable.''",
    },
    geneAlias: {
      type: 'string',
      description:
        'The location in the AnnData store of a different list of names for gene list component, other than the `var` index',
    },
  },
};
