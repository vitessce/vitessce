...
export default class HeatmapCompositeTextLayer extends CompositeLayer {
    ...
    return hideTopLabels
      ...
      : [
        new TextLayer({
          ...
          getText: d => d[1],
          getPosition: d => [
            matrixLeft + ((d[0] + 0.5) / width) * matrixWidth,
            axisLabelTop,
          ],
          getTextAnchor: 'start',
  ...
  _renderCornerLayers() {
    ...
    return cellColorLabelsData.map(
      data => new TextLayer({
        ...
        getPosition: d => [
          transpose
            ? axisLabelLeft
            : targetX
                + ((-cellColorLabelsData.length + d[0] * 2) * COLOR_BAR_SIZE
                  + AXIS_MARGIN)
                  / 2
                  / scaleFactor,
          transpose
    ...
    return hideLeftLabels
      ...
      : [
        new TextLayer({
          ...
          getPosition: d => [
            axisLabelLeft,
            matrixTop + ((d[0] + 0.5) / height) * matrixHeight,
          ],
          getTextAnchor: 'end',
          getColor: () => THEME_TO_TEXT_COLOR[theme],
