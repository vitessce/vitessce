import { z } from 'zod';
import { requestInit, componentCoordinationScopes, componentCoordinationScopesBy, } from './shared.js';
...
function buildFileDefSchema(fileType, options) {
    return z.object({
        ...
        requestInit: requestInit
            .describe('The properties of this object correspond to the parameters of the JavaScript fetch() function.')
            .optional(),
        coordinationValues: z.record(z.string())
            .describe('Keys are coordination types. Values are coordination values. Used for matching views to files.')
            .optional(),
    });
}
/**
 ...
 */
export function buildConfigSchema(pluginFileTypes, pluginJointFileTypes, pluginCoordinationTypes, pluginViewTypes) {
    ...
    return z.object({
        ...
        datasets: z.array(z.object({
            ...
            description: z.string().optional(),
            files: z.array(fileDefs),
        }))
            .describe('The datasets array defines groups of files, where the files within each dataset reference the same entities (cells, genes, cell sets, etc).'),
        // Merge with coordination type schemas.
        coordinationSpace: z.object(
        // Wrap each value schema in z.record()
        Object.fromEntries(pluginCoordinationTypes
            .map(ct => ([
            ct.name,
            z.record(
            ...
            ct.valueSchema.optional()).optional(),
        ]))))
            .strict()
            .describe('The coordination space stores the values for each scope of each coordination object.')
            .optional(),
        layout: z.array(z.object({
            uid: z.string()
                .describe('A unique identifier for the view, to refer to it in getter and setter functions in object-oriented contexts.')
                .optional(),
            component: toEnum(pluginViewTypes.map(vt => vt.name))
                .describe('Specify a component using a name defined in the component registry.'),
            ...
            .describe('The layout array defines the views, or components, rendered in the grid.'),
        initStrategy: z.enum(['none', 'auto'])
            .describe('The initialization strategy determines how missing coordination objects and coordination scope mappings are initially filled in.'),
    });
}
