...
/** @typedef {import('./types').AnyVersionConfig} AnyVersionConfig */
...
export function configSchemaToVersion(zodSchema) {
  ...
  return (/** @type {import('zod').z.AnyZodObject} */ (/** @type {unknown} */ (zodSchema))).shape.version._def.value;
}

/** @type {Record<string, import('zod').z.ZodTypeAny>} */
 ...
 * @param {AnyVersionConfig} config The parsed config.
 * @param {import('zod').z.RefinementCtx} ctx The Zod refinement context.
 */
function refineCoordinationTypes(config, ctx) {
  if ('version' in config) {
    ...
    deprecatedCoordinationTypes.forEach((prevConstant) => {
      const newTypeName = (/** @type {Record<string, string[]>} */(OldCoordinationType))[prevConstant][3];
      const prevName = (/** @type {Record<string, string[]>} */(OldCoordinationType))[prevConstant][0];
      if (
 ...
 * 
 * @param {any} config 
 * @param {((a: any, b: any) => void)|null} [onConfigUpgrade=null]
 * @returns {import('zod').z.infer<typeof latestConfigSchema>}
 */
export function upgradeAndParse(
  config,
  onConfigUpgrade = null,
) {
