...
import { initializeRasterLayersAndChannels, coordinateTransformationsToMatrix, getNgffAxes, loadOmeZarr, guessTileSize, ZarritaPixelSource, } from '@vitessce/spatial-utils';
...
async function initLoader(imageData) {
    const { type, url, metadata, requestInit, } = imageData;
    switch (type) {
        case ('zarr'): {
            ...
            if (isPyramid) {
                ...
                const data = await Promise.all(paths.map(path => zarrOpen(root.resolve(path), { kind: 'array' })));
