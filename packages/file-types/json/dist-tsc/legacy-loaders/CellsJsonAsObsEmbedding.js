...
export default class CellsJsonAsObsEmbeddingLoader extends JsonLoader {
    ...
    loadFromCache(data) {
        ...
        if (cellObjs.length > 0 && (!cellObjs[0].mappings
            ...
            const obsEmbeddingX = Float32Array.from(cellObjs.map(cellObj => cellObj.mappings[embeddingType][0]));
            const obsEmbeddingY = Float32Array.from(cellObjs.map(cellObj => cellObj.mappings[embeddingType][1]));
            const obsEmbedding = {
