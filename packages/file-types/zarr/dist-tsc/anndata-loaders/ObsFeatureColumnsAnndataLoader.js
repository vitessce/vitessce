...
export default class ObsFeatureColumnsAnndataLoader extends AbstractTwoStepLoader {
    ...
    async loadObsFeatureColumns() {
        ...
        if (!this.obsFeatureColumns) {
            ...
            const data = await Promise.all(colPaths.map(colPath => this.dataSource.loadNumeric(colPath)));
    ...
    async load() {
        ...
        return Promise.all([
            ...
            .resolve(new LoaderResult({ obsIndex, obsFeatureMatrix, featureIndex }, null)));
    }
}
