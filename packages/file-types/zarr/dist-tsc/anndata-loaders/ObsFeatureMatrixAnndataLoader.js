...
export default class ObsFeatureMatrixAnndataLoader extends AbstractTwoStepLoader {
    ...
    async _openSparseArrays() {
        ...
        this.sparseArrays = Promise.all(['indptr', 'indices', 'data'].map(name => zarrOpen(storeRoot.resolve(`${matrix}/${name}`), { kind: 'array' })));
        return this.sparseArrays;
    }
    /**
    ...
    async loadGeneSelection({ selection }) {
        ...
        if (encodingType === 'csc_matrix') {
            ...
            genes = await Promise.all(indices.map(index => this.arr
    ...
    async load() {
        ...
        return Promise.all([
            ...
            this.loadCellXGene(),
        ]).then(([obsIndex, featureIndex, obsFeatureMatrix]) => Promise.resolve(new LoaderResult({ obsIndex, featureIndex, obsFeatureMatrix }, null)));
    }
}
