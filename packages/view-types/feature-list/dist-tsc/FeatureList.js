...
export default function FeatureList(props) {
    ...
    const data = useMemo(() => {
        ...
        if (preSortedData && featureListSort === 'alphabetical' && preSortedData.length > 0) {
            return preSortedData.sort((a, b) => a[selectableTableSortKey].localeCompare(b[selectableTableSortKey]));
        }
    ...
    return (_jsxs(_Fragment, { children: [_jsx("input", { className: classes.searchBar, type: "text", placeholder: "Search", value: searchTerm, onChange: handleChange }), _jsx(SelectableTable, { columns: columns, columnLabels: columnLabels, data: data, hasColorEncoding: hasColorEncoding, idKey: "key", valueKey: "value", onChange: onChange, allowMultiple: enableMultiSelect, allowUncheck: enableMultiSelect, showTableHead: columnLabels.length > 1 })] }));
}
