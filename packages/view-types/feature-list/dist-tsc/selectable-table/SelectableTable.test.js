...
describe('SelectableTable.js', () => {
    describe('<SelectableTable />', () => {
        it('renders column values', async () => {
            render(_jsx(SelectableTable, { data: tableData, columns: tableColumns, columnLabels: columnLabels, idKey: tableIdKey, testHeight: 500, testWidth: 500 }));
        ...
        it('emits single selected object when allowMultiple is false', async () => {
            const p = new Promise((resolve) => {
                render(_jsx(SelectableTable, { data: tableData, columns: tableColumns, columnLabels: columnLabels, idKey: tableIdKey, onChange: (selection) => {
                        expect(Array.isArray(selection)).toBe(false);
                        expect(selection.Name).toEqual('Tile 1');
                        resolve();
                    }, testHeight: 500, testWidth: 500 }));
        ...
        it('emits an array of selected objects when allowMultiple is true', async () => {
            const p = new Promise((resolve) => {
                render(_jsx(SelectableTable, { data: tableData, columns: tableColumns, columnLabels: columnLabels, idKey: tableIdKey, allowMultiple: true, onChange: (selection) => {
        ...
        it('renders single column values', async () => {
            ...
            render(_jsx(SelectableTable, { data: tableData, columns: customTableColumns, columnLabels: customColumnLabels, idKey: tableIdKey, testHeight: 500, testWidth: 500 }));
