...
const Heatmap = forwardRef((props, deckRef) => {
    const { uuid, theme, viewState: rawViewState, setViewState, width: viewWidth, height: viewHeight, uint8ObsFeatureMatrix, cellColors, cellColorLabels = [''], colormap, colormapRange, setComponentHover, setCellHighlight = createDefaultUpdateCellsHover('Heatmap'), setGeneHighlight = createDefaultUpdateGenesHover('Heatmap'), setTrackHighlight = createDefaultUpdateTracksHover('Heatmap'), updateViewInfo = createDefaultUpdateViewInfo('Heatmap'), setIsRendering = () => { }, transpose = false, variablesTitle = 'Genes', observationsTitle = 'Cells', variablesDashes = true, observationsDashes = true, useDevicePixels = 1, hideObservationLabels = false, hideVariableLabels = false, onHeatmapClick, setColorEncoding, obsIndex, featureIndex, featureLabelsMap, } = props;
    const viewState = {
        ...rawViewState,
        ...
        if (uint8ObsFeatureMatrix && cellOrdering.length
            ...
            for (let cellOrderingIndex = 0; cellOrderingIndex < cellOrdering.length; cellOrderingIndex += 1) {
                ...
                for (let geneIndex = 0; geneIndex < featureIndex.length; geneIndex += 1) {
                    ...
                    paddedExpressionContainer[newIndex % (DATA_TEXTURE_SIZE * DATA_TEXTURE_SIZE)] = uint8ObsFeatureMatrix[index];
