...
import { TitleInfo, useReady, useClosestVitessceContainerSize, useWindowDimensions, useImageData, useObsLocationsData, useObsSegmentationsData, useCoordination, useLoaders, useAuxiliaryCoordination, useComponentLayout, } from '@vitessce/vit-s';
...
const LayerControllerMemoized = React.memo(forwardRef((props, ref) => {
    const { title, closeButtonVisible, downloadButtonVisible, removeGridComponent, theme, isReady, dataset, obsType, moleculesLayer, setMoleculesLayer, cellsLayer, // May be one polygon layer object or an array of bitmask layers.
    setCellsLayer, rasterLayers, imageLayerLoaders, imageLayerMeta, imageLayerCallbacks, setImageLayerCallbacks, areLoadingImageChannels, setAreLoadingImageChannels, handleRasterLayerChange, handleRasterLayerRemove, obsSegmentationsType, segmentationLayerLoaders, segmentationLayerMeta, segmentationLayerCallbacks, setSegmentationLayerCallbacks, areLoadingSegmentationChannels, setAreLoadingSegmentationChannels, handleSegmentationLayerChange, handleSegmentationLayerRemove, disable3d, globalDisable3d, disableChannelsIfRgbDetected, layerIs3DIndex, setZoom, setTargetX, setTargetY, setTargetZ, setRotationX, setRotationOrbit, componentHeight, componentWidth, spatialLayout, handleImageAdd, enableLayerButtonsWithOneLayer, } = props;
    const shouldShowImageLayerButton = Boolean(enableLayerButtonsWithOneLayer || imageLayerLoaders?.length > 1);
    return (_jsx(TitleInfo, { title: title, isScroll: true, closeButtonVisible: closeButtonVisible, downloadButtonVisible: downloadButtonVisible, removeGridComponent: removeGridComponent, theme: theme, isReady: isReady, children: _jsxs("div", { className: "layer-controller-container", ref: ref, children: [moleculesLayer && (_jsx(VectorLayerController, { label: "Molecules", layerType: "molecules", layer: moleculesLayer, handleLayerChange: setMoleculesLayer }, `${dataset}-molecules`)), cellsLayer && obsSegmentationsType === 'polygon' && (_jsx(VectorLayerController, { label: `${capitalize(obsType)} Segmentations`, layerType: "cells", layer: cellsLayer, handleLayerChange: setCellsLayer }, `${dataset}-cells`)), cellsLayer && obsSegmentationsType === 'bitmask'
                    && cellsLayer.map((layer, i) => {
                        ...
                        return loader && layerMeta ? (_jsx(Grid
                        // eslint-disable-next-line react/no-array-index-key
                        , { item: true, style: { marginTop: '10px' }, children: _jsx(LayerController, { name: layerMeta.name, layer: layer, loader: loader, theme: theme, handleLayerChange: v => handleSegmentationLayerChange(v, i), handleLayerRemove: () => handleSegmentationLayerRemove(i), ChannelController: BitmaskChannelController, shouldShowTransparentColor: isRaster, shouldShowDomain: isRaster, shouldShowColormap: isRaster, 
                                // Disable 3D if given explicit instructions to do so
                                // or if another layer is using 3D mode.
                                disable3d: globalDisable3d
                                    || (disable3d || []).indexOf(layerMeta.name) >= 0
                                    || (typeof layerIs3DIndex === 'number'
                                        && layerIs3DIndex !== -1
                                        && layerIs3DIndex !== i), disabled: typeof layerIs3DIndex === 'number'
                                    && layerIs3DIndex !== -1
                                    && layerIs3DIndex !== i, disableChannelsIfRgbDetected: disableChannelsIfRgbDetected, imageLayerCallbacks: imageLayerCallbacks, setImageLayerCallback: setSegmentationLayerCallback, setViewState: ({ zoom: newZoom, target, rotationX: newRotationX, rotationOrbit: newRotationOrbit, }) => {
                                    setZoom(newZoom);
                                    setTargetX(target[0]);
                                    setTargetY(target[1]);
                                    setTargetZ(target[2]);
                                ...
                                }, setAreLayerChannelsLoading: setAreLayerChannelsLoading, areLayerChannelsLoading: areLayerChannelsLoading, spatialHeight: (componentHeight * (spatialLayout ? spatialLayout.h : 1)) / 12, spatialWidth: (componentWidth * (spatialLayout ? spatialLayout.w : 1)) / 12, shouldShowRemoveLayerButton: shouldShowImageLayerButton }) }, `${dataset}-raster-${index}-${i}`)) : null;
                    }), rasterLayers
                    && rasterLayers.map((layer, i) => {
                        ...
                        return loader && layerMeta ? (_jsx(Grid
                        // eslint-disable-next-line react/no-array-index-key
                        , { item: true, style: { marginTop: '10px' }, children: _jsx(LayerController, { name: layerMeta.name, layer: layer, loader: loader, theme: theme, handleLayerChange: v => handleRasterLayerChange(v, i), handleLayerRemove: () => handleRasterLayerRemove(i), ChannelController: RasterChannelController, shouldShowTransparentColor: isRaster, shouldShowDomain: isRaster, shouldShowColormap: isRaster, 
                                // Disable 3D if given explicit instructions to do so
                                // or if another layer is using 3D mode.
                                disable3d: globalDisable3d
                                    || (disable3d || []).indexOf(layerMeta.name) >= 0
                                    || (typeof layerIs3DIndex === 'number'
                                        ...
                                        && layerIs3DIndex !== i), disabled: typeof layerIs3DIndex === 'number'
                                    && layerIs3DIndex !== -1
                                    && layerIs3DIndex !== i, disableChannelsIfRgbDetected: disableChannelsIfRgbDetected, imageLayerCallbacks: imageLayerCallbacks, setImageLayerCallback: setImageLayerCallback, setViewState: ({ zoom: newZoom, target, rotationX: newRotationX, rotationOrbit: newRotationOrbit, }) => {
                                    setZoom(newZoom);
                                    setTargetX(target[0]);
                                    setTargetY(target[1]);
                                    setTargetZ(target[2]);
                    ...
                    ? (_jsx(Grid, { item: true, children: _jsx(ImageAddButton, { imageOptions: imageLayerMeta, handleImageAdd: handleImageAdd }) })) : null] }) }));
}));
/**
...
export function LayerControllerSubscriber(props) {
    ...
    const [{ image }, imageStatus] = useImageData(loaders, dataset, false, { setSpatialImageLayer: setRasterLayers }, { spatialImageLayer: rasterLayers }, {});
    ...
    return (_jsx(LayerControllerMemoized, { ref: layerControllerRef, title: title, closeButtonVisible: closeButtonVisible, downloadButtonVisible: downloadButtonVisible, removeGridComponent: removeGridComponent, theme: theme, isReady: isReady, moleculesLayer: moleculesLayer, dataset: dataset, obsType: obsType, setMoleculesLayer: setMoleculesLayer, cellsLayer: cellsLayer, setCellsLayer: setCellsLayer, rasterLayers: rasterLayers, imageLayerLoaders: imageLayerLoaders, imageLayerMeta: imageLayerMeta, imageLayerCallbacks: imageLayerCallbacks, setImageLayerCallbacks: setImageLayerCallbacks, areLoadingImageChannels: areLoadingImageChannels, setAreLoadingImageChannels: setAreLoadingImageChannels, handleRasterLayerChange: handleRasterLayerChange, handleRasterLayerRemove: handleRasterLayerRemove, obsSegmentationsType: obsSegmentationsType, segmentationLayerLoaders: segmentationLayerLoaders, segmentationLayerMeta: segmentationLayerMeta, segmentationLayerCallbacks: segmentationLayerCallbacks, setSegmentationLayerCallbacks: setSegmentationLayerCallbacks, areLoadingSegmentationChannels: areLoadingSegmentationChannels, setAreLoadingSegmentationChannels: setAreLoadingSegmentationChannels, handleSegmentationLayerChange: handleSegmentationLayerChange, handleSegmentationLayerRemove: handleSegmentationLayerRemove, disable3d: disable3d, globalDisable3d: globalDisable3d, layerIs3DIndex: layerIs3DIndex, disableChannelsIfRgbDetected: disableChannelsIfRgbDetected, enableLayerButtonsWithOneLayer: enableLayerButtonsWithOneLayer, setZoom: setZoom, setTargetX: setTargetX, setTargetY: setTargetY, setTargetZ: setTargetZ, setRotationX: setRotationX, setRotationOrbit: setRotationOrbit, 
        // Fall back to window for height and width.
        componentHeight: componentHeight || windowHeight, componentWidth: componentWidth || windowWidth, spatialLayout: spatialLayout, handleImageAdd: handleImageAdd }));
}
