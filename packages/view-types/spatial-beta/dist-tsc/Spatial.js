...
import { deck, viv, getSelectionLayer, ScaledExpressionExtension, } from '@vitessce/gl';
import { filterSelection } from '@vitessce/spatial-utils';
import { PALETTE, getDefaultColor } from '@vitessce/utils';
import { setObsSelection as setObsSelectionHelper, treeToCellSetColorIndicesBySetNames, mergeObsSets, getCellColors, } from '@vitessce/sets-utils';
...
const makeDefaultGetCellColorsFromIndices = (setColorObj, obsIndex, theme) => (object, { index }) => {
    ...
    const [r, g, b, a] = (setColorIndices && obsIndex && setColors?.[setColorIndices?.get(obsIndex[index])]?.color) || getDefaultColor(theme);
    return [r, g, b, 255 * (a || 1)];
};
...
class Spatial extends AbstractSpatialOrScatterplot {
    ...
    createPolygonSegmentationLayer(layerScope, layerCoordination, channelScopes, channelCoordinations, layerObsSegmentations, layerFeatureValues) {
        ...
        const { obsColorEncoding, spatialChannelColor, spatialChannelOpacity, spatialChannelVisible, spatialSegmentationFilled, spatialSegmentationStrokeWidth, featureValueColormap, featureValueColormapRange, } = channelCoordination;
    ...
    createSpotLayer(layerScope, layerCoordination, layerObsSpots, layerFeatureData) {
        ...
        const { spatialLayerVisible, spatialLayerOpacity, spatialSpotRadius, spatialSpotFilled, spatialSpotStrokeWidth, obsColorEncoding, featureValueColormap, featureValueColormapRange, spatialLayerColor, } = layerCoordination;
    ...
    createSelectionLayer() {
        const { 
        // Spots
        spotLayerScopes, obsSpots, spotLayerCoordination, 
        // Points
        pointLayerScopes, obsPoints, pointLayerCoordination, 
        // Segmentations
        segmentationLayerScopes, segmentationChannelScopesByLayer, obsSegmentationsLocations, segmentationLayerCoordination, segmentationChannelCoordination, } = this.props;
        ...
        return getSelectionLayer(tool, viewState.zoom, 'nothing', [
            ...pointLayerScopes
            ...
            ...spotLayerScopes
                ...
                onSelect: (obsIds) => {
                    const { obsSetColor, additionalObsSets, } = spotLayerCoordination?.[0]?.[layerScope] || {};
                    const { setObsSetSelection, setObsColorEncoding, setObsSetColor, setAdditionalObsSets, } = spotLayerCoordination?.[1]?.[layerScope] || {};
                    setObsSelectionHelper(obsIds, additionalObsSets, obsSetColor, setObsSetSelection, setAdditionalObsSets, setObsSetColor, setObsColorEncoding);
                },
            })),
            ...segmentationLayerScopes
                ...
                .filter(channelScope => (segmentationChannelCoordination?.[0]?.[layerScope]?.[channelScope]?.[CoordinationType.SPATIAL_CHANNEL_VISIBLE]
                ...
                onSelect: (obsIds) => {
                    const { obsSetColor, additionalObsSets, } = segmentationChannelCoordination?.[0]?.[layerScope]?.[channelScope] || {};
                    const { setObsSetSelection, setObsColorEncoding, setObsSetColor, setAdditionalObsSets, } = segmentationChannelCoordination?.[1]?.[layerScope]?.[channelScope] || {};
                    setObsSelectionHelper(obsIds, additionalObsSets, obsSetColor, setObsSetSelection, setAdditionalObsSets, setObsSetColor, setObsColorEncoding);
                },
            }))),
        ]);
    ...
    createScaleBarLayer() {
        const { viewState, width, height, } = this.props;
        ...
        if (!viewState || !width || !height || use3d)
            return null;
    ...
    use3d() {
        const { spatialRenderingMode, } = this.props;
        return spatialRenderingMode === '3D';
    }
    // New createImageLayer function.
    createBitmaskSegmentationLayer(layerScope, layerCoordination, channelScopes, channelCoordination, image, layerFeatureValues, layerMatrixObsIndices, bitmaskValueIsIndex) {
        ...
        return new viv.MultiscaleImageLayer({
            ...
            channelIsSetColorMode: channelScopes
                .map(cScope => (channelCoordination[cScope][CoordinationType.OBS_COLOR_ENCODING] === 'cellSetSelection')),
    ...
    // New createImageLayer function.
    createImageLayer(layerScope, layerCoordination, channelScopes, channelCoordination, image) {
        const { delegateHover, targetT, targetZ, spatialRenderingMode, } = this.props;
        ...
        const contrastLimits = isRgb ? ([
            ...
            [0, 255],
            [0, 255],
        ]) : channelScopes.map(cScope => (channelCoordination[cScope][CoordinationType.SPATIAL_CHANNEL_WINDOW]
            || ([0, 255])));
    ...
    createImageLayers() {
        const { imageLayerCallbacks = [], images = {}, imageLayerScopes, imageLayerCoordination, imageChannelScopesByLayer, imageChannelCoordination, } = this.props;
        return imageLayerScopes.map(layerScope => this.createImageLayer(layerScope, imageLayerCoordination[0][layerScope], imageChannelScopesByLayer[layerScope], imageChannelCoordination[0][layerScope], images[layerScope]));
    }
    createSpotLayers() {
        const { obsSpots = {}, spotLayerScopes, spotLayerCoordination, spotMultiExpressionData, } = this.props;
        return spotLayerScopes.map((layerScope) => {
            if (obsSpots[layerScope]) {
                return this.createSpotLayer(layerScope, spotLayerCoordination[0][layerScope], obsSpots[layerScope], spotMultiExpressionData?.[layerScope]);
            }
            return null;
        });
    }
    createPointLayers() {
        const { obsPoints = {}, pointLayerScopes, pointLayerCoordination, } = this.props;
        return pointLayerScopes.map((layerScope) => {
            if (obsPoints[layerScope]) {
                return this.createPointLayer(layerScope, pointLayerCoordination[0][layerScope], obsPoints[layerScope]);
            }
            return null;
        });
    }
        ...
        return segmentationLayerScopes.map((layerScope) => {
            if (obsSegmentations[layerScope]) {
                ...
                if (obsSegmentationsType === 'bitmask') {
                    return this.createBitmaskSegmentationLayer(layerScope, segmentationLayerCoordination[0][layerScope], segmentationChannelScopesByLayer[layerScope], segmentationChannelCoordination[0][layerScope], obsSegmentations[layerScope], segmentationMultiExpressionData?.[layerScope], this.segmentationToMatrixIndexArr?.[layerScope], 
                    // TODO: get this from the layer coordination.
                    bitmaskValueIsIndex);
                }
                if (obsSegmentationsType === 'polygon') {
                    return this.createPolygonSegmentationLayer(layerScope, segmentationLayerCoordination[0][layerScope], segmentationChannelScopesByLayer[layerScope], segmentationChannelCoordination[0][layerScope], obsSegmentations[layerScope], segmentationMultiExpressionData?.[layerScope]);
                }
            }
            return null;
        });
    ...
    getLayers() {
        const { imageLayers, obsSpotsLayers, obsPointsLayers, obsSegmentationsLayers,
        // neighborhoodsLayer,
        // obsLocationsLayer,
         } = this;
        return [
    ...
    onUpdateSpotsSetsData(layerScope) {
        const { obsSpots, obsSpotsSets, spotLayerCoordination, theme, } = this.props;
        const { obsSets: layerSets, obsIndex: layerIndex } = obsSpotsSets?.[layerScope] || {};
        if (layerSets && layerIndex) {
            const { obsSetColor, obsColorEncoding, obsSetSelection, additionalObsSets, } = spotLayerCoordination[0][layerScope];
    ...
    onUpdateAllSpotsSetsData() {
        const { spotLayerScopes, } = this.props;
    ...
    onUpdateSpotsIndexData(layerScope) {
        const { obsSpots, spotMatrixIndices, } = this.props;
    ...
    onUpdateSpotsExpressionData(layerScope) {
        const { spotMultiExpressionData, } = this.props;
    ...
    onUpdateSegmentationsSetsData(layerScope, channelScope) {
        const { obsSegmentations, obsSegmentationsSets, segmentationLayerCoordination, segmentationChannelCoordination, theme, } = this.props;
        ...
        if (layerSets && layerIndex) {
            ...
            if (obsSetColor !== prevSetColor || obsSetSelection !== prevSetSelection) {
                ...
                const obsColorIndices = treeToCellSetColorIndicesBySetNames(mergedLayerSets, obsSetSelection, obsSetColor);
    ...
    onUpdateSegmentationsIndexData(layerScope, channelScope) {
        const { obsSegmentations, segmentationMatrixIndices, } = this.props;
    ...
    onUpdateSegmentationsExpressionData(layerScope, channelScope) {
        const { segmentationMultiExpressionData, } = this.props;
    ...
    onUpdateSegmentationsLocationsData(layerScope, channelScope) {
        ...
        if (layerObsSegmentations && obsSegmentationsType === 'polygon') {
            if (layerObsLocations && layerObsLocations.shape[1] === layerObsSegmentations.shape[0]) {
                ...
                this.obsSegmentationsQuadTree[layerScope][channelScope] = createQuadTree(layerObsLocations, getCellCoords);
            }
        }
    }
    ...
    onUpdateSegmentationsData(layerScope) {
        const { obsSegmentations, } = this.props;
        const { obsSegmentations: layerObsSegmentations, obsSegmentationsType } = obsSegmentations?.[layerScope] || {};
        if (layerObsSegmentations && obsSegmentationsType === 'polygon') {
    ...
    onUpdateAllSegmentationsData() {
        const { segmentationLayerScopes, } = this.props;
        segmentationLayerScopes?.forEach((layerScope) => {
    ...
    onUpdateAllPointsData() {
        const { pointLayerScopes, } = this.props;
    ...
    */
    ...
    viewInfoDidUpdate() {
        ...
        super.viewInfoDidUpdate(null, null, makeDefaultGetObsCoords);
    }
    recenter() {
    ...
    componentDidUpdate(prevProps) {
        ...
        const shallowDiffByChannel = (propName, firstName, secondName) => (prevProps?.[propName]?.[firstName]?.[secondName]
            !== this.props?.[propName]?.[firstName]?.[secondName]);
        const shallowDiffByLayerCoordination = (propName, layerScope) => (prevProps?.[propName]?.[0]?.[layerScope]
            !== this.props?.[propName]?.[0]?.[layerScope]);
        const shallowDiffByChannelCoordination = (propName, layerScope, channelScope) => (prevProps?.[propName]?.[0]?.[layerScope]?.[channelScope]
        ...
        if (['segmentationLayerScopes', 'segmentationChannelScopesByLayer'].some(shallowDiff)) {
            ...
            this.props.segmentationLayerScopes?.forEach((layerScope) => {
                this.props.segmentationChannelScopesByLayer?.[layerScope]?.forEach((channelScope) => {
                    if (shallowDiffByChannel('obsSegmentationsSets', layerScope, channelScope)
        ...
        if (shallowDiff('spotLayerScopes')) {
            ...
            this.props.spotLayerScopes.forEach((layerScope) => {
                if (shallowDiffByLayer('obsSpotsSets', layerScope)
        ...
        if (shallowDiff('spotLayerScopes')) {
            ...
            this.props.spotLayerScopes.forEach((layerScope) => {
                if (shallowDiffByLayer('spotMatrixIndices', layerScope)
                    || shallowDiffByLayer('obsSpots', layerScope)) {
                    this.onUpdateSpotsIndexData(layerScope);
                    forceUpdate = true;
                }
        ...
        if (shallowDiff('pointLayerScopes')) {
            ...
            this.props.pointLayerScopes.forEach((layerScope) => {
                if (shallowDiffByLayer('obsPoints', layerScope)
 ...
 */
const SpatialWrapper = forwardRef((props, deckRef) => (_jsx(Spatial, { ...props, deckRef: deckRef })));
