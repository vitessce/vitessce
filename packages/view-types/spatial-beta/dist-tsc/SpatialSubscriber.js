...
import { TitleInfo, useDeckCanvasSize, useReady, useUrls, useMultiObsSpots, useMultiObsPoints, useSpotMultiObsSets, useMultiObsSegmentations, useMultiImages, usePointMultiObsLabels, useSpotMultiFeatureSelection, useSpotMultiObsFeatureMatrixIndices, useSegmentationMultiFeatureSelection, useSegmentationMultiObsFeatureMatrixIndices, useSegmentationMultiObsLocations, useSegmentationMultiObsSets, useInitialCoordination, useCoordination, useLoaders, useMergeCoordination, useSetComponentHover, useSetComponentViewInfo, useComplexCoordination, useMultiCoordinationScopesNonNull, useMultiCoordinationScopesSecondaryNonNull, useComplexCoordinationSecondary, useCoordinationScopes, useCoordinationScopesBy, useSpotMultiFeatureLabels, useGridItemSize, } from '@vitessce/vit-s';
...
export function SpatialSubscriber(props) {
    const { uuid, coordinationScopes: coordinationScopesRaw, coordinationScopesBy: coordinationScopesByRaw, closeButtonVisible, downloadButtonVisible, removeGridComponent, observationsLabelOverride, subobservationsLabelOverride: subobservationsLabel = 'molecule', theme, disableTooltip = false, title = 'Spatial', bitmaskValueIsIndex = false, // TODO: move to coordination type
    three: threeFor3d = false, } = props;
    ...
    const [{ dataset, obsType, featureType, featureValueType, spatialZoom: zoom, spatialTargetX: targetX, spatialTargetY: targetY, spatialTargetZ: targetZ, spatialTargetT: targetT, spatialRenderingMode, spatialRotationX: rotationX, spatialRotationY: rotationY, spatialRotationZ: rotationZ, spatialRotationOrbit: rotationOrbit, spatialOrbitAxis: orbitAxis, spatialAxisFixed, 
    // TODO: get obsSets per-layer or per-channel
    additionalObsSets, obsSetColor, obsColorEncoding, obsSetSelection, }, { setSpatialZoom: setZoom, setSpatialTargetX: setTargetX, setSpatialTargetY: setTargetY, setSpatialTargetZ: setTargetZ, setSpatialRotationX: setRotationX, setSpatialRotationY: setRotationY, setSpatialRotationZ: setRotationZ, setSpatialRotationOrbit: setRotationOrbit, 
    // TODO: get obsSets per-layer or per-channel
    setAdditionalObsSets, setObsSetColor, setObsColorEncoding, setObsSetSelection, }] = useCoordination(COMPONENT_COORDINATION_TYPES[ViewType.SPATIAL_BETA], coordinationScopes);
    const { spatialZoom: initialZoom, spatialTargetX: initialTargetX, spatialTargetY: initialTargetY, spatialTargetZ: initialTargetZ, } = useInitialCoordination(COMPONENT_COORDINATION_TYPES[ViewType.SPATIAL_BETA], coordinationScopes);
    ...
    const [imageLayerScopes, imageChannelScopesByLayer] = useMultiCoordinationScopesSecondaryNonNull(CoordinationType.IMAGE_CHANNEL, CoordinationType.IMAGE_LAYER, coordinationScopes, coordinationScopesBy);
    const spotLayerScopes = useMultiCoordinationScopesNonNull(CoordinationType.SPOT_LAYER, coordinationScopes);
    ...
    ], coordinationScopes, coordinationScopesBy, CoordinationType.IMAGE_LAYER, CoordinationType.IMAGE_CHANNEL);
    // Spot layer
    ...
    */
    ...
    const { initialTargetX: defaultTargetX, initialTargetY: defaultTargetY, initialTargetZ: defaultTargetZ, initialZoom: defaultZoom, } = useMemo(() => getInitialSpatialTargets({
        width,
        height,
    ...
    const setViewState = ({ zoom: newZoom, target, rotationX: newRotationX, rotationOrbit: newRotationOrbit, }) => {
        setZoom(newZoom);
        setTargetX(target[0]);
        setTargetY(target[1]);
        if (is3dMode) {
    ...
    */
    ...
    const isSelectable = (spotLayerScopes.length > 0
    ...
    return (_jsxs(TitleInfo, { title: title, info: subtitle, isSpatial: true, urls: urls, theme: theme, closeButtonVisible: closeButtonVisible, downloadButtonVisible: downloadButtonVisible, removeGridComponent: removeGridComponent, isReady: isReady, children: [shouldUseThree ? (_jsx(SpatialThreeAdapter, { ref: threeRef, uuid: uuid, width: width, height: height, theme: theme, hideTools: !isSelectable, rotation: [rotationX, rotationY, rotationZ], setRotationX: setRotationX, setRotationY: setRotationY, setRotationZ: setRotationZ, 
                // Global view state
                targetT: targetT, targetZ: targetZ, viewState: isValidViewState ? ({
                    zoom,
                    target: [targetX, targetY, targetZ],
                    rotationX,
                    rotationOrbit,
                }) : DEFAULT_VIEW_STATE, orbitAxis: orbitAxis, spatialAxisFixed: spatialAxisFixed, setViewState: isValidViewState ? setViewState : SET_VIEW_STATE_NOOP, originalViewState: originalViewState, spatialRenderingMode: spatialRenderingMode, updateViewInfo: setComponentViewInfo, delegateHover: delegateHover, onEntitySelected: onEntitySelected, 
                // Points
                obsPoints: obsPointsData, pointLayerScopes: pointLayerScopes, pointLayerCoordination: pointLayerCoordination, pointMultiObsLabels: pointMultiObsLabelsData, 
                // Spots
                obsSpots: obsSpotsData, spotLayerScopes: spotLayerScopes, spotLayerCoordination: spotLayerCoordination, obsSpotsSets: obsSpotsSetsData, spotMatrixIndices: spotMultiIndicesData, spotMultiExpressionData: spotMultiExpressionNormData, 
                // Segmentations
                segmentationLayerScopes: segmentationLayerScopes, segmentationLayerCoordination: segmentationLayerCoordination, segmentationChannelScopesByLayer: segmentationChannelScopesByLayer, segmentationChannelCoordination: segmentationChannelCoordination, obsSegmentations: obsSegmentationsData, obsSegmentationsLocations: obsSegmentationsLocationsData, obsSegmentationsSets: obsSegmentationsSetsData, segmentationMatrixIndices: segmentationMultiIndicesData, segmentationMultiExpressionData: segmentationMultiExpressionNormData, bitmaskValueIsIndex: bitmaskValueIsIndex, 
                // Images
                images: imageData, imageLayerScopes: imageLayerScopes, imageLayerCoordination: imageLayerCoordination, imageChannelScopesByLayer: imageChannelScopesByLayer, imageChannelCoordination: imageChannelCoordination })) : (_jsx(Spatial, { ref: deckRef, uuid: uuid, width: width, height: height, theme: theme, hideTools: !isSelectable, 
                // Global view state
                targetT: targetT, targetZ: targetZ, viewState: isValidViewState ? ({
                    zoom,
                    target: [targetX, targetY, targetZ],
                    rotationX,
                    rotationOrbit,
                }) : DEFAULT_VIEW_STATE, orbitAxis: orbitAxis, spatialAxisFixed: spatialAxisFixed, setViewState: isValidViewState ? setViewState : SET_VIEW_STATE_NOOP, originalViewState: originalViewState, spatialRenderingMode: spatialRenderingMode, updateViewInfo: setComponentViewInfo, delegateHover: delegateHover, 
                // Points
                obsPoints: obsPointsData, pointLayerScopes: pointLayerScopes, pointLayerCoordination: pointLayerCoordination, pointMultiObsLabels: pointMultiObsLabelsData, 
                // Spots
                obsSpots: obsSpotsData, spotLayerScopes: spotLayerScopes, spotLayerCoordination: spotLayerCoordination, obsSpotsSets: obsSpotsSetsData, spotMatrixIndices: spotMultiIndicesData, spotMultiExpressionData: spotMultiExpressionNormData, 
                ...
                segmentationLayerScopes: segmentationLayerScopes, segmentationLayerCoordination: segmentationLayerCoordination, segmentationChannelScopesByLayer: segmentationChannelScopesByLayer, segmentationChannelCoordination: segmentationChannelCoordination, obsSegmentations: obsSegmentationsData, obsSegmentationsLocations: obsSegmentationsLocationsData, obsSegmentationsSets: obsSegmentationsSetsData, segmentationMatrixIndices: segmentationMultiIndicesData, segmentationMultiExpressionData: segmentationMultiExpressionNormData, bitmaskValueIsIndex: bitmaskValueIsIndex, 
                // Images
                images: imageData, imageLayerScopes: imageLayerScopes, imageLayerCoordination: imageLayerCoordination, imageChannelScopesByLayer: imageChannelScopesByLayer, imageChannelCoordination: imageChannelCoordination })), !disableTooltip && (_jsx(SpatialTooltipSubscriber, { parentUuid: uuid, width: width, height: height, hoverCoord: hoverCoord, 
                // Points
                obsPoints: obsPointsData, pointLayerScopes: pointLayerScopes, pointLayerCoordination: pointLayerCoordination, 
                // Spots
                obsSpots: obsSpotsData, spotLayerScopes: spotLayerScopes, spotLayerCoordination: spotLayerCoordination, 
                // Segmentations
                obsSegmentationsLocations: obsSegmentationsLocationsData, segmentationLayerScopes: segmentationLayerScopes, segmentationChannelScopesByLayer: segmentationChannelScopesByLayer, segmentationChannelCoordination: segmentationChannelCoordination, 
                // Images
                imageLayerScopes: imageLayerScopes, imageLayerCoordination: imageLayerCoordination })), _jsx(MultiLegend
            // Fix to dark theme due to black background of spatial plot.
            , { 
                // Fix to dark theme due to black background of spatial plot.
                theme: "dark", 
                // Segmentations
                segmentationLayerScopes: segmentationLayerScopes, segmentationLayerCoordination: segmentationLayerCoordination, segmentationChannelScopesByLayer: segmentationChannelScopesByLayer, segmentationChannelCoordination: segmentationChannelCoordination, segmentationMultiExpressionExtents: segmentationMultiExpressionExtents, 
                // Spots
                spotLayerScopes: spotLayerScopes, spotLayerCoordination: spotLayerCoordination, spotMultiExpressionExtents: spotMultiExpressionExtents, spotMultiFeatureLabels: obsSpotsFeatureLabelsData, 
                // Points
                pointLayerScopes: pointLayerScopes, pointLayerCoordination: pointLayerCoordination }), _jsx(ChannelNamesLegend
            // Images
            , { 
                // Images
                images: imageData, imageLayerScopes: imageLayerScopes, imageLayerCoordination: imageLayerCoordination, imageChannelScopesByLayer: imageChannelScopesByLayer, imageChannelCoordination: imageChannelCoordination })] }));
}
