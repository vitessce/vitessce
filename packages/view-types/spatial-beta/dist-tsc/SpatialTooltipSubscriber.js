...
function TooltipChild(props) {
    const { x, y, parentUuid, sourceUuid, width, height, info, tooltipsVisible, tooltipCrosshairsVisible, } = props;
    const visible = parentUuid === sourceUuid ? tooltipsVisible : tooltipCrosshairsVisible;
    return (visible ? (_jsx(Tooltip2D, { x: x, y: y, parentUuid: parentUuid, sourceUuid: sourceUuid, parentWidth: width, parentHeight: height, children: _jsx(TooltipContent, { info: info }) })) : null);
}
...
export default function SpatialTooltipSubscriber(props) {
    const { parentUuid, width, height, hoverCoord, 
    // Points
    obsPoints, pointLayerScopes, pointLayerCoordination, 
    // Spots
    obsSpots, spotLayerScopes, spotLayerCoordination, 
    // Segmentations
    obsSegmentationsLocations, segmentationLayerScopes, segmentationChannelScopesByLayer, segmentationChannelCoordination, 
    // Images
    imageLayerScopes, imageLayerCoordination, } = props;
    ...
    let yOffset = -30;
    return (_jsxs(_Fragment, { children: [projectedHoverCoord && imageLayerScopes?.map((layerScope) => {
                const { pixelHighlight, tooltipsVisible, spatialLayerVisible, } = imageLayerCoordination?.[0]?.[layerScope] || {};
                return (tooltipsVisible && pixelHighlight && spatialLayerVisible ? (_jsx(TooltipChild, { parentUuid: parentUuid, sourceUuid: sourceUuid, width: width, height: height, info: {
                        'Pixel Value': JSON.stringify(pixelHighlight),
                    }, x: projectedHoverCoord?.[0], y: projectedHoverCoord?.[1] + (yOffset += 30) }, layerScope)) : null);
                ...
                if (!xy)
                    return null;
                const [x, y] = xy;
                return (_jsx(TooltipChild, { parentUuid: parentUuid, sourceUuid: sourceUuid, tooltipsVisible: tooltipsVisible, tooltipCrosshairsVisible: tooltipCrosshairsVisible, width: width, height: height, info: {
                        [`${capitalize(obsType)} ID`]: obsHighlight,
                    }, x: x, y: y + (yOffset += 30) }, `${layerScope}-${channelScope}`));
