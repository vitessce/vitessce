...
import { TitleInfo, useReady, useUrls, useGridItemSize, useCoordination, useLoaders, useGenomicProfilesData, } from '@vitessce/vit-s';
...
export function GenomicProfilesSubscriber(props) {
    const { coordinationScopes, theme, closeButtonVisible, downloadButtonVisible, removeGridComponent, profileTrackUidKey = 'path', profileTrackNameKey = null, higlassServer = 'https://higlass.io/api/v1', assembly = 'hg38', title = 'Genomic Profiles', showGeneAnnotations = true, } = props;
    // eslint-disable-next-line no-unused-vars
    const [width, height, containerRef] = useGridItemSize();
    const loaders = useLoaders();
    // Get "props" from the coordination space.
    const [{ dataset, obsSetColor: cellSetColor, obsSetSelection: cellSetSelection, }] = useCoordination(COMPONENT_COORDINATION_TYPES[ViewType.GENOMIC_PROFILES], coordinationScopes);
    ...
    return (_jsx("div", { className: classes.higlassTitleWrapper, children: _jsx(TitleInfo, { title: title, closeButtonVisible: closeButtonVisible, downloadButtonVisible: downloadButtonVisible, removeGridComponent: removeGridComponent, theme: theme, isReady: isReady, urls: urls, children: _jsx("div", { className: classes.higlassLazyWrapper, ref: containerRef, children: hgViewConfig ? (_jsx(HiGlassLazy, { coordinationScopes: coordinationScopes, theme: theme, hgViewConfig: hgViewConfig, height: height })) : null }) }) }));
}
