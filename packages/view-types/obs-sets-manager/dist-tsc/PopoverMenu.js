...
function PopoverMenuListButton(props) {
    const { title, subtitle, onClick, handler, handlerKey, confirm, visible, } = props;
    ...
    return (_jsxs("button", { title: titleWithConfirm, type: "button", onClick: handleOrRequireConfirm, onKeyPress: e => callbackOnKeyPress(e, handlerKey, handleOrRequireConfirm), children: [titleWithConfirm, subtitle && (_jsxs(_Fragment, { children: [_jsx("br", {}), _jsx("span", { className: classes.small, children: subtitle })] }))] }));
}
/**
...
function PopoverMenuList(props) {
    const { menuConfig, onClick, color = null, palette = null, setColor = null, visible, } = props;
    ...
    const classes = useHelpTooltipStyles();
    return (_jsxs("div", { children: [color && setColor && defaultPalette && (_jsx(TwitterPicker, { className: classes.popoverMenuColor, disableAlpha: true, width: 108, triangle: "hide", colors: defaultPalette, color: colorArrayToString(color), onChangeComplete: handleColorChange })), _jsx("ul", { className: classes.popoverMenuList, children: menuConfig.map(item => (_jsx("li", { children: _jsx(PopoverMenuListButton, { ...item, onClick: onClick, visible: visible }) }, item.title + item.subtitle))) })] }));
}
/**
...
export default function PopoverMenu(props) {
    const { menuConfig, placement, children, color = null, setColor = null, palette = null, } = props;
    const [visible, setVisible] = useState(false);
    return (_jsx(Popover, { content: (_jsx(PopoverMenuList, { menuConfig: menuConfig, onClick: () => setVisible(false), color: color, setColor: setColor, palette: palette, visible: visible })), placement: placement, visible: visible, onVisibleChange: setVisible, children: children }));
}
