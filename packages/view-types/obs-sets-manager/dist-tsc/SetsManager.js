...
export default function SetsManager(props) {
    ...
    function renderTreeNodes(nodes, readOnly, currPath) {
        ...
        return nodes.map((node) => {
            ...
            return (_jsx(TreeNode, { theme: theme, ...nodeToRenderProps(node, newPath, setColor), isEditing: isEqual(isEditingNodeName, newPath), datatype: datatype, draggable: draggable && !readOnly, editable: editable && !readOnly, checkable: checkable, expandable: expandable, exportable: exportable, hasColorEncoding: hasColorEncoding, isChecking: isChecking, checkedLevelPath: checkedLevel ? checkedLevel.levelZeroPath : null, checkedLevelIndex: checkedLevel ? checkedLevel.levelIndex : null, onCheckNode: onCheckNode, onCheckLevel: onCheckLevel, onNodeView: onNodeView, onNodeSetColor: onNodeSetColor, onNodeSetName: (targetPath, name) => {
                    onNodeSetName(targetPath, name);
                    setIsEditingNodeName(null);
                }, onNodeCheckNewName: onNodeCheckNewName, onNodeSetIsEditing: setIsEditingNodeName, onNodeRemove: onNodeRemove, onExportLevelZeroNodeJSON: onExportLevelZeroNodeJSON, onExportLevelZeroNodeTabular: onExportLevelZeroNodeTabular, onExportSetJSON: onExportSetJSON, disableTooltip: isDragging, onDragStart: () => setIsDragging(true), onDragEnd: () => setIsDragging(false), children: renderTreeNodes(node.children, readOnly, newPath, theme) }, pathToKey(newPath)));
        });
    }
    const classes = useStyles();
    return (_jsxs("div", { className: classes.setsManager, children: [_jsxs("div", { className: classes.setsManagerTree, children: [_jsx(Tree, { draggable: false, checkable: checkable, checkedKeys: setSelectionKeys, expandedKeys: setExpansionKeys, autoExpandParent: autoExpandParent, onCheck: (checkedKeys, info) => onCheckNode(info.node.props.nodeKey, info.checked), onExpand: (expandedKeys, info) => onExpandNode(expandedKeys, info.node.props.nodeKey, info.expanded), children: renderTreeNodes(processedSets.tree, true, [], theme) }), _jsx(Tree, { draggable /* TODO */: true, checkable: checkable, checkedKeys: additionalSetSelectionKeys, expandedKeys: additionalSetExpansionKeys, autoExpandParent: autoExpandParent, onCheck: (checkedKeys, info) => onCheckNode(info.node.props.nodeKey, info.checked), onExpand: (expandedKeys, info) => onExpandNode(expandedKeys, info.node.props.nodeKey, info.expanded), onDrop: (info) => {
                        ...
                        }, children: renderTreeNodes(processedAdditionalSets.tree, false, [], theme) }), _jsx(PlusButton, { datatype: datatype, onError: onError, onImportTree: onImportTree, onCreateLevelZeroNode: onCreateLevelZeroNode, importable: importable, editable: editable })] }), isChecking ? (_jsx("div", { className: classes.setOperationButtons, children: _jsx(SetOperationButtons, { onUnion: onUnion, onIntersection: onIntersection, onComplement: onComplement, operatable: operatable, hasCheckedSetsToUnion: hasCheckedSetsToUnion, hasCheckedSetsToIntersect: hasCheckedSetsToIntersect, hasCheckedSetsToComplement: hasCheckedSetsToComplement }) })) : null] }));
}
