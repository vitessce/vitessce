...
export function ObsSetsManagerSubscriber(props) {
    ...
    const onExpandNode = useCallback((expandedKeys, targetKey, expanded) => {
        ...
        if (expanded) {
            ...
            const newCellSetExpansion = prevCellSetExpansion.filter(d => !isEqual(d, targetKey.split(PATH_SEP)));
    ...
    return (_jsx(TitleInfo, { title: title, isScroll: true, closeButtonVisible: closeButtonVisible, downloadButtonVisible: downloadButtonVisible, removeGridComponent: removeGridComponent, urls: urls, theme: theme, isReady: isReady, children: manager }));
}
