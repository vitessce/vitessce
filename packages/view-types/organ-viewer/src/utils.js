...
export async function getBlocksFromOrgan(organName, gender) {
    // console.log(organName)
    // Get Mapping between organ model and the UBERON IDs
    // let subPathToLabel = await getObjSubPathToOntology();


    // Get the ontoloy to know which UBERON ID we are searching for:
    // let ontology = await getOntology();
    // let kidney = ontology.get("http://purl.obolibrary.org/obo/UBERON_0002113")
    // console.log(kidney);
    // kidney.children.forEach(child => {
    //     // all kidney related structures
    //     console.log(ontology.get(child).label + ";" + ontology.get(child).synonym);
    // })
            ...
            data.forEach(d => {
                if (d["entityId"]?.includes("hubmap")) {
                    if (d["ccf_annotations"].includes(organName)) { //Only for left Kidney
                        // console.log(d)
                        entitiesCollection.set(d["entityId"].split("/").slice(-1)[0], {
                            entityId: d["entityId"].split("/").slice(-1)[0], annotations: d["ccf_annotations"],
                            transformationMatrix: d["transformMatrix"], color: d["color"],
                        })
                    }
                }
            })
    ...
    // console.log(hubmapEntities)

    let blocks = Array.from(hubmapEntities.entries()).map(value => {
        let entity = value[1];
        return {
            id: value[0],
    ...
    return {blocks: blocks, hubmapEntities: hubmapEntities, ccfEntities: ccfEntities};
}
        ...
        .then(data => {
            ...
            data.forEach(d => {
                if (d.representation_of === organUberon) {
                    retVal = {matrix: d.transformMatrix, uberon: d.representation_of}
                }
            });
            return retVal;
        });
