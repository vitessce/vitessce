...
import { abbreviateNumber, toRgbUIString, DOMAINS, } from '@vitessce/spatial-utils';
...
export default function ChannelSlider(props) {
    const { image, color, window, setWindow, showValueExtent, disabled, minMaxDomain, colormapOn, theme, } = props;
    ...
    const handleChangeDebounced = useCallback(debounce(setWindow || (() => { }), 3, { trailing: true }), [setWindow]);
    return (_jsx(Slider, { value: window || [0, 0], valueLabelFormat: abbreviateNumber, onChange: (e, v) => handleChangeDebounced(v), valueLabelDisplay: "auto", getAriaLabel: (index) => {
            const labelPrefix = index === 0 ? 'Low value slider' : 'High value slider';
            return `${labelPrefix} for ${rgbColor} colormap channel`;
        }, getAriaValueText: () => `Current colormap values: ${rgbColor}-${window}`, min: min, max: max, step: step, orientation: "horizontal", className: classes.channelSlider, style: { color: rgbColor }, disabled: disabled }));
}
