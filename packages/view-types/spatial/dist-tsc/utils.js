...
export function getInitialSpatialTargets({ width, height, obsCentroids, obsSegmentations, obsSegmentationsType, imageLayerLoaders, useRaster, use3d, modelMatrices, }) {
    ...
    if (imageLayerLoaders.length > 0 && useRaster) {
        for (let i = 0; i < imageLayerLoaders.length; i += 1) {
            ...
            const { target, zoom: newViewStateZoom } = viv.getDefaultInitialViewState(imageLayerLoaders[i].data, viewSize, zoomBackoff, use3d, new Matrix4(modelMatrices[i]));
    ...
    else if (!useRaster && ((obsSegmentationsType === 'polygon' && obsSegmentations)
...
export function renderSubBitmaskLayers(props) {
    const { bbox: { left, top, right, bottom, }, index: { x, y, z }, } = props.tile;
