...
import { TitleInfo, useDeckCanvasSize, useReady, useUrls, useObsLocationsData, useObsSegmentationsData, useObsSetsData, useFeatureSelection, useImageData, useObsFeatureMatrixIndices, useFeatureLabelsData, useNeighborhoodsData, useObsLabelsData, useMultiObsLabels, useUint8FeatureSelection, useExpressionValueGetter, useGetObsInfo, useInitialCoordination, useCoordination, useLoaders, useSetComponentHover, useSetComponentViewInfo, useAuxiliaryCoordination, useHasLoader, } from '@vitessce/vit-s';
import { setObsSelection, mergeObsSets, colorArrayToString, getCellColors, } from '@vitessce/sets-utils';
...
export function SpatialSubscriber(props) {
    ...
    const hasSegmentationsLoader = useHasLoader(loaders, dataset, DataType.OBS_SEGMENTATIONS, { obsType });
    const hasLocationsData = useHasLoader(loaders, dataset, DataType.OBS_LOCATIONS, { obsType });
    ...
    const hasSegmentationsData = hasSegmentationsLoader && !(obsSegmentationsStatus === STATUS.SUCCESS
    ...
    const [{ image }, imageStatus, imageUrls] = useImageData(loaders, dataset, false, { setSpatialImageLayer: setRasterLayers }, { spatialImageLayer: imageLayers }, {});
    ...
    const setViewState = ({ zoom: newZoom, target, rotationX: newRotationX, rotationOrbit: newRotationOrbit, orbitAxis: newOrbitAxis, }) => {
        setZoom(newZoom);
        setTargetX(target[0]);
        setTargetY(target[1]);
    ...
    const options = useMemo(() => {
        ...
        if (canShow3DOptions || hasExpressionData) {
            return (_jsx(SpatialOptions, { observationsLabel: observationsLabel, cellColorEncoding: cellColorEncoding, setCellColorEncoding: setCellColorEncoding, setSpatialAxisFixed: setSpatialAxisFixed, spatialAxisFixed: spatialAxisFixed, use3d: use3d, tooltipsVisible: tooltipsVisible, setTooltipsVisible: setTooltipsVisible, geneExpressionColormap: geneExpressionColormap, setGeneExpressionColormap: setGeneExpressionColormap, geneExpressionColormapRange: geneExpressionColormapRange, setGeneExpressionColormapRange: setGeneExpressionColormapRange, canShowExpressionOptions: hasExpressionData, canShowColorEncodingOption: (hasLocationsData || hasSegmentationsData) && hasExpressionData, canShow3DOptions: canShow3DOptions }));
        }
        return null;
    ...
    const [channelNames, channelColors] = useMemo(() => {
        ...
        if (imageLayers && imageLayers.length > 0
            ...
            if (firstImageLayer && !firstImageLayer.colormap && firstImageLayer.channels
    ...
    return (_jsxs(TitleInfo, { title: title, info: subtitle, isSpatial: true, urls: urls, theme: theme, closeButtonVisible: closeButtonVisible, downloadButtonVisible: downloadButtonVisible, removeGridComponent: removeGridComponent, isReady: isReady, options: options, children: [_jsx("div", { style: {
                    position: 'absolute',
                    bottom: '5px',
                    left: '5px',
                    zIndex: 6,
                }, children: channelNamesVisible && channelNames ? channelNames.map((name, i) => (_jsx(Typography, { variant: "h6", style: {
                        color: colorArrayToString(channelColors[i]),
                        fontSize: '14px',
                    }, children: name }, `${name}-${colorArrayToString(channelColors[i])}`))) : null }), _jsx(Spatial, { ref: deckRef, uuid: uuid, width: width, height: height, viewState: {
                    zoom,
                    ...
                    rotationOrbit,
                    orbitAxis,
                }, setViewState: setViewState, originalViewState: originalViewState, imageLayerDefs: imageLayers, obsSegmentationsLayerDefs: cellsLayer, obsLocationsLayerDefs: moleculesLayer, neighborhoodLayerDefs: neighborhoodsLayer, obsLocationsIndex: obsLocationsIndex, obsSegmentationsIndex: obsSegmentationsIndex, obsLocations: obsLocations, obsLocationsLabels: obsLocationsLabels, obsLocationsFeatureIndex: obsLocationsFeatureIndex, hasSegmentations: hasSegmentationsData, obsSegmentations: obsSegmentations, obsSegmentationsType: obsSegmentationsType, obsCentroids: obsCentroids, obsCentroidsIndex: obsCentroidsIndex, cellFilter: cellFilter, cellSelection: cellSelection, cellHighlight: cellHighlight, cellColors: cellColors, neighborhoods: neighborhoods, imageLayerLoaders: resolutionFilteredImageLayerLoaders, setCellFilter: setCellFilter, setCellSelection: setCellSelectionProp, setCellHighlight: setCellHighlight, setHoverInfo: setHoverInfo, setMoleculeHighlight: setMoleculeHighlight, setComponentHover: () => {
                    setComponentHover(uuid);
                }, updateViewInfo: setComponentViewInfo, imageLayerCallbacks: imageLayerCallbacks, segmentationLayerCallbacks: segmentationLayerCallbacks, spatialAxisFixed: spatialAxisFixed, geneExpressionColormap: geneExpressionColormap, geneExpressionColormapRange: geneExpressionColormapRange, expressionData: shiftedExpressionDataForBitmask, cellColorEncoding: cellColorEncoding, getExpressionValue: getExpressionValue, theme: theme, useFullResolutionImage: useFullResolutionImage }), tooltipsVisible && (_jsx(SpatialTooltipSubscriber, { parentUuid: uuid, obsHighlight: cellHighlight, width: width, height: height, getObsInfo: getObsInfo, useHoverInfoForTooltip: useHoverInfoForTooltip, hoverData: hoverData, hoverCoord: hoverCoord, getObsIdFromHoverData: getObsIdFromHoverData })), _jsx(Legend, { visible: true, 
                // Fix to dark theme due to black background of spatial plot.
                theme: "dark", featureType: featureType, featureValueType: featureValueType, obsColorEncoding: cellColorEncoding, featureSelection: geneSelection, featureLabelsMap: featureLabelsMap, featureValueColormap: geneExpressionColormap, featureValueColormapRange: geneExpressionColormapRange, extent: expressionExtents?.[0] })] }));
}
