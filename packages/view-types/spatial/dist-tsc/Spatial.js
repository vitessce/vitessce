...
class Spatial extends AbstractSpatialOrScatterplot {
    ...
    createPolygonSegmentationsLayer(layerDef, hasExplicitPolygons) {
        ...
        const { obsCentroidsIndex, obsSegmentationsIndex, } = this.props;
        const obsIndex = (hasExplicitPolygons ? obsSegmentationsIndex : obsCentroidsIndex);
        const { theme, 
        // cellFilter,
        cellSelection, setCellHighlight, setComponentHover, getCellIsSelected = makeDefaultGetCellIsSelected(obsIndex.length === cellSelection.length ? null : cellSelection), cellColors, getCellColor = makeDefaultGetCellColors(cellColors, obsIndex, theme), onCellClick, lineWidthScale = 10, lineWidthMaxPixels = 2, geneExpressionColormapRange, cellColorEncoding, getExpressionValue, geneExpressionColormap, } = this.props;
    ...
    createMoleculesLayer(layerDef) {
        const { obsLocations, obsLocationsFeatureIndex: obsLabelsTypes, setMoleculeHighlight, } = this.props;
    ...
    createNeighborhoodsLayer(layerDef) {
        const { getNeighborhoodPolygon = (neighborhoodsEntry) => {
    ...
    createScaleBarLayer() {
        const { viewState, width, height, imageLayerLoaders = {}, imageLayerDefs, } = this.props;
    ...
    createRasterLayer(rawLayerDef, loader, i) {
        ...
        if (rawLayerDef.type === 'bitmask') {
            const { geneExpressionColormap, geneExpressionColormapRange = [0.0, 1.0], cellColorEncoding, } = this.props;
        ...
        const extensions = getVivLayerExtensions(layerDef.use3d, layerProps.colormap, layerProps.renderingMode);
    ...
    createImageLayers() {
        const { imageLayerDefs, imageLayerLoaders = {}, imageLayerCallbacks = [], } = this.props;
        const use3d = this.use3d();
        const use3dIndex = (imageLayerDefs || []).findIndex(i => i.use3d);
        return (imageLayerDefs || [])
            .filter(layer => (use3d ? layer.use3d === use3d : true))
            .map((layer, i) => this.createRasterLayer({ ...layer, callback: imageLayerCallbacks[use3d ? use3dIndex : i] }, imageLayerLoaders[layer.index], i));
    }
    createBitmaskLayers() {
        const { obsSegmentationsLayerDefs, obsSegmentations, obsSegmentationsType, segmentationLayerCallbacks = [], } = this.props;
        if (obsSegmentations && obsSegmentationsType === 'bitmask' && Array.isArray(obsSegmentationsLayerDefs)) {
            ...
            return obsSegmentationsLayerDefs
                .filter(layer => (use3d ? layer.use3d === use3d : true))
                .map((layer, i) => this.createRasterLayer({ ...layer, callback: segmentationLayerCallbacks[use3d ? use3dIndex : i] }, loaders[layer.index], i));
        }
        return [];
    }
    getLayers() {
        const { imageLayers, obsSegmentationsPolygonLayer, neighborhoodsLayer, obsLocationsLayer, obsSegmentationsBitmaskLayers, } = this;
        return [
            ...imageLayers,
    ...
    onUpdateCellsLayer() {
        const { obsSegmentationsLayerDefs: obsSegmentationsLayerDef, obsSegmentationsIndex, obsSegmentations, obsSegmentationsType, obsCentroids, obsCentroidsIndex, hasSegmentations, } = this.props;
        if (obsSegmentationsLayerDef && obsSegmentationsIndex && obsSegmentations && obsSegmentationsType === 'polygon') {
            this.obsSegmentationsPolygonLayer = this.createPolygonSegmentationsLayer(obsSegmentationsLayerDef, true);
        }
            ...
            this.obsSegmentationsPolygonLayer = this.createPolygonSegmentationsLayer(obsSegmentationsLayerDef, false);
        }
        else {
            this.obsSegmentationsPolygonLayer = null;
        }
    ...
    onUpdateMoleculesData() {
        const { obsLocations, obsLocationsLabels: obsLabels, obsLocationsFeatureIndex: obsLabelsTypes, } = this.props;
    ...
    onUpdateMoleculesLayer() {
        const { obsLocationsLayerDefs: obsLocationsLayerDef, obsLocations, obsLocationsIndex, obsLocationsLabels, obsLocationsFeatureIndex, } = this.props;
        if (obsLocationsLayerDef
    ...
    viewInfoDidUpdate() {
        const { obsCentroidsIndex, obsCentroids, } = this.props;
        super.viewInfoDidUpdate(obsCentroidsIndex, obsCentroids, makeDefaultGetObsCoords);
    }
    recenter() {
