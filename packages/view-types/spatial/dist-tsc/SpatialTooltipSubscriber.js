...
export default function SpatialTooltipSubscriber(props) {
    const { parentUuid, obsHighlight, width, height, getObsInfo, hoverData, hoverCoord, useHoverInfoForTooltip, getObsIdFromHoverData, } = props;
    const sourceUuid = useComponentHover();
    const viewInfo = useComponentViewInfo(parentUuid);
    let [cellInfo, x, y] = [null, null, null];
    if (useHoverInfoForTooltip && getObsIdFromHoverData
