...
import { Scene, Group, MeshPhysicalMaterial, MeshBasicMaterial, MeshStandardMaterial, BackSide, FrontSide, } from 'three';
...
import { useVolumeSettings, create3DRendering, initialDataLoading, } from './three-utils.js';
/**
...
export function SpatialThree(props) {
    ...
    if (obsSegmentations?.[layerScope]?.obsSegmentations && segmentationGroup == null) {
        ...
        if (scene?.children) {
            ...
            scene.children.forEach((child) => {
                ...
                simplified.geometry.scale(sceneOptions?.scaleX ?? 1.0, sceneOptions?.scaleY ?? 1.0, sceneOptions?.scaleZ ?? 1.0);
            ...
            newScene.scale.set(sceneOptions?.sceneScaleX ?? 1.0, sceneOptions?.sceneScaleY ?? 1.0, sceneOptions?.sceneScaleZ ?? 1.0);
            const sceneScale = [
    ...
    if (volumeSettings.is3dMode && (!renderingSettings.uniforms || !renderingSettings.shader)) {
        return (_jsxs("group", { children: [_jsx("ambientLight", {}), _jsx("pointLight", { position: [10, 10, 10] }), _jsx(Text, { color: "white", scale: 20, fontWeight: 1000, children: "Loading ..." })] }));
    }
    ...
    return (_jsxs("group", { children: [_jsx(Controllers, {}), _jsx(Hands, {}), _jsx(HandBbox, {}), _jsx(HandDecorate, {}), _jsx(GeometryAndMesh, { ...geometryAndMeshProps }), _jsx(OrbitControls, { ref: orbitRef, enableDamping: false, dampingFactor: 0.0, zoomDampingFactor: 0.0, smoothZoom: false })] }));
}
