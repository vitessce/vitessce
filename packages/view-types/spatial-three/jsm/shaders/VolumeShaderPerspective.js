...
var VolumeRenderShaderPerspective = {
    ...
    vertexShader: [
        ...
        "varying vec2 vUv;",
        "varying mat4 projectionMatrixOut;",
        "varying mat4 viewMatrixOut;",
        "void main()",
        "{",
    ...
    fragmentShader: [
        ...
        // "  float depth = readDepth(u_geo_depth, vec2(x,y));",
        // "  if(depth < 0.1)",
        // "    depth = 0.0;",
        "  for (float t = t_hit.x; t < t_hit.y; t += dt) {",
        // "      float viewZ = perspectiveDepthToViewZ(p.z, near, far );",
        // "      float depthValue = viewZToOrthographicDepth( viewZ, near, far);",
        // "      float depthInData = ((t-t_hit.x) / (t_hit.y-t_hit.x)*2.0);",
        //
        // "      vec4 depth_vec = viewMatrixOut * projectionMatrixOut * vec4(p.xyz, 1.0);",
        // "      float depthInData = ((depth_vec.z / depth_vec.w) + 1.0) * 0.5;",
        //
        // "      if (depth<depthInData) {", // Compose the already synced up Additive render with the Geometric rendering results
        // "           vec4 val_color = texture2D(u_geo_color, vec2(x,y));",
        ...
        "       total += val6;",
        "       }",
        "       if(u_renderstyle == 0 && (max_val > u_clim[1] && max_val2 >= u_clim2[1] && max_val3 >= u_clim3[1] && max_val4 >= u_clim4[1] && max_val5 >= u_clim5[1] &&  max_val6 >= u_clim6[1])) {" +
        "           break;",
        "       }",
        "       if(u_renderstyle == 2){" +
        "           total = min(total, 1.0);",
