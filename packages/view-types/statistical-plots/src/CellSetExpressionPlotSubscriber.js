...
export function CellSetExpressionPlotSubscriber(props) {
  ...
  const [
    ...
    { setFeatureValueTransform, setFeatureValueTransformCoefficient },
  ] = useCoordination(
  ...
  return (
    <TitleInfo
      title={`Expression by ${capitalize(obsType)} Set${
        firstGeneSelected ? ` (${firstGeneSelected})` : ''
      }`}
