...
export function useFilteredVolcanoData(props) {
  ...
  const filteredData = useMemo(() => computedData.map((d) => ({
    ...d,
    // Instead of an object of one array per column,
    // this is now an array of one object per row.
    df: d.df.featureId.map((featureId, i) => ({
      featureId,
      logFoldChange: d.df.logFoldChange[i],
      featureSignificance: d.df.featureSignificance[i],
      minusLog10p: d.df.minusLog10p[i],
      // TODO: include the obsSetName.
    })).filter(d => (
