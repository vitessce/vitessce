...
const PEAK_HEATMAP_DATA_TYPES = [DataType.CELLS, DataType.CELL_SETS, DataType.PEAK_MATRIX];
 ...
 */
export default function PeakHeatmapSubscriber(props) {
  const {
    uuid,
    coordinationScopes,
    ...
    title = `${capitalize(variablesLabel)} Count Heatmap`,
  } = props;
  ...
  const [{
    ...
    cellHighlight,
    peakHighlight: geneHighlight,
    cellSetSelection,
    cellSetColor,
    additionalCellSets,
    peakCountColormap: geneExpressionColormap,
    peakCountColormapRange: geneExpressionColormapRange,
  }, {
    ...
    setCellSetSelection,
    setCellSetColor,
    setPeakCountColormapRange: setGeneExpressionColormapRange,
    setPeakCountColormap: setGeneExpressionColormap,
  }] = useCoordination(COMPONENT_COORDINATION_TYPES[Component.PEAK_HEATMAP], coordinationScopes);
  ...
  const [expressionMatrix] = usePeakMatrixData(
