...
export default class InMemoryMatrixLoader extends JsonLoader {
  async load() {
    const data = {
      rows: Array.from({ length: 9000 }, (i, j) => `gene_${j}`),
      cols: Array.from({ length: 6000 }, (i, j) => `cell_${j}`),
      matrix: Array.from({ length: 9000 }, (i, j) =>
        Array.from({ length: 6000 }, (l, k) => Math.floor((k * 9000 + j) / (4096*4096)) / 3)
      ),
    };
