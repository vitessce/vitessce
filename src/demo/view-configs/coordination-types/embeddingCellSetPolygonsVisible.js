...
function getConfig() {
  ...
  const v1 = vc.addView(dataset, vapi.cm.SCATTERPLOT, { mapping: 't-SNE', x: 0, y: 0, w: 5, h: 6 });
  const v2 = vc.addView(dataset, vapi.cm.SCATTERPLOT, { mapping: 't-SNE', x: 5, y: 0, w: 5, h: 6 });
  const v3 = vc.addView(dataset, vapi.cm.SCATTERPLOT, { mapping: 't-SNE', x: 0, y: 6, w: 5, h: 6 });
  const v4 = vc.addView(dataset, vapi.cm.SCATTERPLOT, { mapping: 't-SNE', x: 5, y: 6, w: 5, h: 6 });
  const cellSets = vc.addView(dataset, vapi.cm.CELL_SETS, { x: 10, y: 0, w: 2, h: 12 });
