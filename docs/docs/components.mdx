...
Users select two genes, and a scatterplot is dynamically generated using the `expression-matrix` data. Gating can then be performed by using the lasso or box select tools.
Use with datasets containing the `cells` data type and `expression-matrix` data types. Optionally, the `cell-sets` and `expression-matrix` data types can be used for coloring cells on the plot.
