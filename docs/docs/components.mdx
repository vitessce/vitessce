...
The observation set sizes view displays a bar plot with the currently-selected observation sets (e.g., cell sets) (using the `obsSets` data type and [`obsSetSelection`](/docs/coordination-types/#obsSetSelection) coordination type) on the x-axis and bars representing their size (by number of cells) on the y-axis. This view was implemented with [Vega-Lite](https://vega.github.io/vega-lite/).
