...
An observation-x-feature matrix stored in a CSV file.
...
An observation-x-feature matrix with observations along the `obs` axis (rows) and features along the `var` axis (columns).
...
const dataset = vc
  ...
  .addFile(
    undefined,
    ft.OBS_SEGMENTATIONS_RASTER_JSON,
    {
      obsType: "cell"
    },
    options
