...
Currently, Vitessce requires that the data type of the matrix is `uint8` (sometimes abbreviated `u1`), to improve performance.
In the future, we hope to remove this requirement and support float data types with the ability to perform multiple types of normalization on-the-fly within Vitessce.
Therefore, we recommend using the [layers](https://anndata.readthedocs.io/en/latest/generated/anndata.AnnData.layers.html) feature of AnnData to store a copy of `adata.X` that has been normalized and cast to `uint8`, while keeping `adata.X` with its original dtype:
...
See the [JSON schema](https://github.com/vitessce/vitessce/blob/main/src/schemas/obsSets.schema.json) and an [example](https://s3.amazonaws.com/vitessce-data/0.0.31/master_release/dries/dries.cell-sets.json) for reference.
...
[OME-TIFF](https://docs.openmicroscopy.org/ome-model/6.0.0/index.html#ome-tiff) images are supported via the [`image.raster.json`](#imagerasterjson) file type.
...
:::tip
The [`ome-zarr`](https://pypi.org/project/ome-zarr/) Python package can be used to read the metadata of  `OME-NGFF` images.
:::