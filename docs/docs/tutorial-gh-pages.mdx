...
title: 'Tutorial: Deploy Vitessce instance GitHub Pages'
sidebar_label: GitHub Pages Deployment
slug: /tutorial-gh-pages
---

This tutorial covers how to embed Vitessce as a React component in a website that can be deployed to the GitHub Pages free static website hosting service.
...
AWS S3 is one example of a static file hosting service (object storage system) that can be configured such that other websites (e.g., GitHub Pages sites) can load its hosted files.
...
However, in this tutorial we will describe how to use a [GitHub Action](https://docs.github.com/en/actions) to automatically re-deploy the React app to the GitHub Pages website on every push to the main branch of its GitHub repository.
...
To allow the GitHub Action to modify modify the `gh-pages` branch of the repository, we need to provide it with a Personal access token that has the appropriate permissions.
