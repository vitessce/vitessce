...
In parallel, the single-cell community is generalizing the cell-by-gene matrix from transcriptomics to represent other types of experimental data using "observation-by-feature" matrices (denoted **FOM** in the [Feature and Observation Matrix Schema Working Group](https://github.com/single-cell-data/mams/) terminology).
...
Vitessce was tightly coupled to the more specific cell-by-gene ideas, preventing straightforward use with non-transcriptomics or multi-modal datasets.
...
In addition, we might have run a [topic model-based](https://doi.org/10.1038/s41592-019-0367-1) analysis on our data, so we also have a cell-by-topic matrix:
```js
dataset: 'my_atac',
obsType: 'cell',
featureType: 'topic',
featureValueType: 'probability'
```
...
The `viewCoordinationValues` item refers to the coordination values that a particular view takes on at a given point in time.
...
The changes described in this blog post are reflected in the view config schema [versions](/docs/view-config-json/#version) `1.0.13` and above.
You can [diff](https://observablehq.com/@keller-mark/vitessce-config-version-diff?ref=main&prev=1.0.10&next=1.0.13) new and old versions to see the changes.
...
This is a breaking change, but it should be easy to update existing code to pass a `schemaVersion` value (see [Migration](#migration) section below).

For new code, we recommend specifying the latest view config schema version:

```js
const vc = new VitessceConfig("1.0.15", "My config");
```
...
We hope that these changes make it easier to configure Vitessce, [prepare data](/docs/data-file-types/) for use with Vitessce, and develop [plugins](/docs/dev-plugins/).
While we want to make Vitessce general enough to facilitate many use cases, we recognize the danger of over abstraction (e.g., preventing users from easily configuring Vitessce), and hope that our implementation strikes a useful balance.
...
We also intend for the observation-by-feature paradigm to simplify the conceptualization and implementation of additional functionality.
...
We are considering how to enable this use case by supporting mappings between `obsType`s to indicate which cells belong to which samples (if a dataset contains multiple tissue sample observations with per-sample cellular observations).
Such mappings could also be specified in the opposite direction to capture which organelles belong to which cells.
Again, if you have feedback about this or other features, we would love to hear!
...
The remaining sections are meant for developers looking to make existing code compatible with the latest version of the `vitessce` JavaScript package.

### Deprecated constant values
...
If you are already using the `VitessceConfig` class in existing code (package version `1.2.0` or earlier), and you would like to upgrade to the new version while keeping the previous behavior:
...
The parameters of the [`addFile`](/docs/view-config-js/#addfileurl-filetype-coordinationvalues-options) method have changed.
Data type is no longer an argument, so file type has moved to the second argument.
The new argument `coordinationValues` is now the third argument.
The fourth argument is now `options`.
The `coordinationValues` and `options` parameters are optional: `null` can be passed when not applicable.

```diff
- dataset.addFile('http://example.com/my_cells.json', dt.CELLS, ft.CELLS_JSON);
+ dataset.addFile('http://example.com/my_cells.json', ft.CELLS_JSON, null, null);
```
