...
describe('loaders/ObsFeatureMatrixAnndataLoader', () => {
  describe('AnnData v0.7', () => {
    ...
    it('loadGeneSelection matches across storage methods', async () => {
      ...
      const loaderCsr = createMatrixLoader('http://localhost:51204/@fixtures/zarr/anndata-0.7/anndata-csr.zarr');
      const csrSelection = await loaderCsr.loadGeneSelection(selection);
      const loaderDense = createMatrixLoader('http://localhost:51204/@fixtures/zarr/anndata-0.7/anndata-dense.zarr');
      const denseSelection = await loaderDense.loadGeneSelection(selection);
      const loaderCsc = createMatrixLoader('http://localhost:51204/@fixtures/zarr/anndata-0.7/anndata-csc.zarr');
  ...
  describe('AnnData v0.8', () => {
    ...
    it('loadGeneSelection matches across storage methods', async () => {
      const selection = { selection: ['gene_1', 'gene_5'] };
      const loaderCsr = createMatrixLoader('http://localhost:51204/@fixtures/zarr/anndata-0.8/anndata-csr.zarr');
      const csrSelection = await loaderCsr.loadGeneSelection(selection);
      const loaderDense = createMatrixLoader('http://localhost:51204/@fixtures/zarr/anndata-0.8/anndata-dense.zarr');
      const denseSelection = await loaderDense.loadGeneSelection(selection);
      const loaderCsc = createMatrixLoader('http://localhost:51204/@fixtures/zarr/anndata-0.8/anndata-csc.zarr');
      const cscSelection = await loaderCsc.loadGeneSelection(selection);
      expect(cscSelection).toEqual(denseSelection);
      expect(csrSelection).toEqual(denseSelection);
    });
